...
describe('OperationPool', () => {
  ...
  describe('Constructor and Configuration', () => {
    test('should create pool with default config', () => {
      const defaultPool = new OperationPool()
    ...
    test('should create pool with custom config', () => {
      const customPool = new OperationPool({
        concurrency: 5,
        retries: 5,
        retryDelay: 500,
        timeout: 5000
    ...
    test('should normalize concurrency to 10 if invalid', () => {
      const invalidPool = new OperationPool({ concurrency: -1 })
      expect(invalidPool.concurrency).toBe(10)
      invalidPool.stop()
    })

    test('should handle "auto" concurrency', () => {
      const autoPool = new OperationPool({ concurrency: 'auto' })
  ...
  describe('Concurrency Limit', () => {
    ...
    test('should allow concurrency > items', async () => {
      const largePool = new OperationPool({ concurrency: 100 })
  ...
  describe('Retry Logic', () => {
    ...
    test('should not retry non-retryable errors', async () => {
      const selectivePool = new OperationPool({
        concurrency: 2,
        retries: 3,
  ...
  describe('Timeout Handling', () => {
    test('should timeout slow tasks', async () => {
      const fastPool = new OperationPool({ concurrency: 2, timeout: 50 })
  ...
  describe('Metrics Collection', () => {
    ...
    test('should disable metrics collection when configured', async () => {
      const noMetricsPool = new OperationPool({
        concurrency: 2,
