...
describe('System Overhead and Effective Limit', () => {
  test('calculateSystemOverhead with default config', () => {
    const overhead = calculateSystemOverhead();
    expect(typeof overhead).toBe('number');
    expect(overhead).toBeGreaterThan(0);
  });

  test('calculateSystemOverhead with timestamps and id', () => {
    const overhead = calculateSystemOverhead({
      version: '10',
      timestamps: true,
      id: 'abc123'
  ...
  test('calculateSystemOverhead with custom version', () => {
    const overhead = calculateSystemOverhead({
      version: '100'
    });
    expect(typeof overhead).toBe('number');
  ...
  test('calculateEffectiveLimit with default config', () => {
    const limit = calculateEffectiveLimit();
    expect(typeof limit).toBe('number');
    expect(limit).toBeLessThanOrEqual(2048);
  });

  test('calculateEffectiveLimit with custom s3Limit and systemConfig', () => {
    const limit = calculateEffectiveLimit({
      s3Limit: 4096,
      systemConfig: { version: '2', timestamps: true, id: 'xyz' }
    });
    expect(typeof limit).toBe('number');
    expect(limit).toBeLessThanOrEqual(4096);
  });

  test('calculateEffectiveLimit with minimal systemConfig', () => {
    const limit = calculateEffectiveLimit({
      s3Limit: 100,
...
describe('Coverage for calculateUTF8Bytes edge case', () => {
  test('should return 0 for empty string (codePointAt returns undefined)', () => {
    expect(calculateUTF8Bytes('')).toBe(0);
  });
}); 

describe('calculateUTF8Bytes non-string input', () => {
  test('should convert non-string input to string and calculate bytes', () => {
    expect(calculateUTF8Bytes(123)).toBe(3); // "123"
    expect(calculateUTF8Bytes(true)).toBe(4); // "true"
    expect(calculateUTF8Bytes({})).toBe(15); // "[object Object]"
    expect(calculateUTF8Bytes(null)).toBe(4); // "null"
    expect(calculateUTF8Bytes(undefined)).toBe(9); // "undefined"
  });
}); 