...
describe('Resource Versioning with Partitions', () => {
  ...
  beforeEach(async () => {
    testPrefix = `test-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    ...
    // Mock the client to avoid actual S3 calls
    const mockStorage = new Map(); // Simple in-memory storage for test
    
    database.client = {
      config: { bucket: 'test-bucket' },
      parallelism: 10,
      exists: jest.fn().mockResolvedValue(false),
      getObject: jest.fn().mockImplementation(async (key) => {
        const data = mockStorage.get(key);
        return {
          Body: { 
            transformToByteArray: () => Promise.resolve(Buffer.from(data?.body || ''))
          },
          ContentLength: data?.body?.length || 0,
          ContentType: data?.contentType || 'application/json'
        };
      }),
      putObject: jest.fn().mockImplementation(async ({ key, body, metadata, contentType }) => {
        mockStorage.set(key, { body: body || '', metadata: metadata || {}, contentType });
        return {};
      }),
      headObject: jest.fn().mockImplementation(async (key) => {
        const data = mockStorage.get(key);
        if (!data) {
