 ...
 * @module mcp/search/hybrid-search
 */

import Fuse from 'fuse.js';
import { cosineSimilarity, combineScores, stringSimilarity } from './math.js';

/**
 * @typedef {import('./types.js').IndexedDoc} IndexedDoc
 * @typedef {import('./types.js').SearchResult} SearchResult
 * @typedef {import('./types.js').SearchOptions} SearchOptions
 * @typedef {import('./types.js').HybridSearchConfig} HybridSearchConfig
...
export class HybridSearch {
  /**
   * @param {EmbeddingEntry[]} documents - Documents with embeddings
   * @param {HybridSearchConfig} [config] - Search configuration
   */
   ...
   * @param {number[]} [queryVector] - Pre-computed query embedding vector
   * @param {SearchOptions} [options] - Search options
   * @returns {SearchResult[]} - Ranked search results
   */
  search(query, queryVector = null, options = {}) {
   ...
   * @returns {SearchResult[]} - Search results
   */
  semanticSearch(queryVector, options = {}) {
    const { limit = 10, category, minScore = 0 } = options;
   ...
   * @private
   */
  _semanticSearch(queryVector, documents) {
   ...
   * @private
   */
  _combineResults(fuzzyResults, semanticResults) {
   ...
   * @private
   */
  _formatResult(result, query, sourceOverride = null) {
