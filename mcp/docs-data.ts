...
export const plugins: PluginDoc[] = [
  ...
  },
  {
    ...
    description: 'FSM for workflow management with guards and actions. Order status, approval flows.',
    configOptions: [
      { name: 'states', type: 'StateConfig[]', required: true, description: 'State definitions' },
      { name: 'transitions', type: 'TransitionConfig[]', required: true, description: 'Allowed transitions' },
    ],
    methods: [
      { name: 'transition', signature: '(id: string, event: string) => Promise<void>', description: 'Trigger state transition' },
      { name: 'canTransition', signature: '(id: string, event: string) => boolean', description: 'Check if transition is allowed' },
    ],
