...
export const plugins: PluginDoc[] = [
  ...
  },
  {
    name: 'S3QueuePlugin',
    category: 'integration',
    description: 'Distributed job queue using S3 as backend, with retries, DLQ, custom metadata and partitions for per-tenant queues.',
    configOptions: [
      { name: 'resource', type: 'string', required: true, description: 'Target resource name' },
      { name: 'visibilityTimeout', type: 'number', required: false, default: '30000', description: 'Message visibility timeout (ms)' },
      { name: 'maxAttempts', type: 'number', required: false, default: '3', description: 'Max retry attempts' },
      { name: 'metadata', type: 'Record<string, string>', required: false, description: 'Extra attributes on queue entries (e.g., clientId)' },
      { name: 'partitions', type: 'Record<string, PartitionConfig>', required: false, description: 'Custom S3 partition indexes for efficient counting/querying' },
    ],
    methods: [
      { name: 'enqueue', signature: '(data: any, options?: { metadata?: Record<string, unknown> }) => Promise<Record>', description: 'Add message to queue with optional metadata' },
      { name: 'countQueueBy', signature: '(filter: Record<string, unknown>) => Promise<number>', description: 'Count messages by partition filter (e.g., { clientId: "acme" })' },
      { name: 'queueStatsBy', signature: '(filter: Record<string, unknown>) => Promise<QueueStats>', description: 'Get status breakdown filtered by partition fields' },
      { name: 'startProcessing', signature: '(handler: MessageHandler) => void', description: 'Start processing messages' },
      { name: 'queueStats', signature: '() => Promise<QueueStats>', description: 'Get queue statistics (total, pending, processing, completed, failed, dead)' },
    ],
    examples: ['e31-s3-queue.js'],
    relatedPlugins: ['ReplicatorPlugin', 'QueueConsumerPlugin'],
  },
  {
    name: 'QueueConsumerPlugin',
    category: 'integration',
    description: 'Consume from external queues (SQS, RabbitMQ, Redis List/Stream/PubSub, BullMQ) and route to S3DB resources or custom handlers.',
    configOptions: [
      ...
      { name: 'logLevel', type: 'string', required: false, default: 'info', description: 'Log level' },
    ],
    methods: [
      { name: 'stop', signature: '() => Promise<void>', description: 'Stop all consumers' },
      { name: 'publish', signature: '(target: string, data: unknown) => Promise<unknown>', description: 'Publish to a named consumer' },
      { name: 'listPublishers', signature: '() => string[]', description: 'List registered consumer names' },
    ],
    examples: ['e27-queue-consumer.js'],
    relatedPlugins: ['S3QueuePlugin', 'ReplicatorPlugin'],
  },
  {
  ...
  },
  {
    ...
    description: 'Workflow orchestration with states, guards, actions, triggers, retries, persistence, and resource.state.* helpers.',
    configOptions: [
      { name: 'stateMachines', type: 'Record<string, MachineConfig>', required: true, description: 'Named machine definitions keyed by machine id' },
      { name: 'actions', type: 'Record<string, ActionHandler>', required: false, description: 'Named entry/exit action handlers' },
      { name: 'guards', type: 'Record<string, GuardHandler>', required: false, description: 'Named transition guard handlers' },
      { name: 'persistTransitions', type: 'boolean', required: false, default: 'true', description: 'Persist current state and transition history' },
      { name: 'resource', type: 'string | Resource', required: false, description: 'Attach a machine to a resource to enable resource.state.* helpers' },
      { name: 'stateField', type: 'string', required: false, description: 'Resource field that mirrors the current machine state' },
      { name: 'autoCleanup', type: 'boolean', required: false, default: 'true', description: 'Auto-delete machine state/history when the attached record is deleted' },
      { name: 'retryConfig', type: 'RetryConfig', required: false, description: 'Retry policy for action execution' },
    ],
    methods: [
      { name: 'send', signature: '(machineId: string, entityId: string, event: string, context?: object) => Promise<TransitionResult>', description: 'Trigger a transition through the machine API' },
      { name: 'getState', signature: '(machineId: string, entityId: string) => Promise<string>', description: 'Get current state for an entity' },
      { name: 'getValidEvents', signature: '(machineId: string, entityIdOrState: string) => Promise<string[]>', description: 'Get valid events for a state or entity' },
      { name: 'getTransitionHistory', signature: '(machineId: string, entityId: string, options?: { limit?: number; offset?: number }) => Promise<TransitionHistoryEntry[]>', description: 'Get persisted transition history' },
      { name: 'initializeEntity', signature: '(machineId: string, entityId: string, context?: object) => Promise<string>', description: 'Initialize entity with the machine initial state' },
      { name: 'deleteEntity', signature: '(machineId: string, entityId: string) => Promise<void>', description: 'Delete entity state and transition history' },
      { name: 'waitForPendingEvents', signature: '(timeout?: number) => Promise<void>', description: 'Wait for async trigger handlers to finish' },
    ],
