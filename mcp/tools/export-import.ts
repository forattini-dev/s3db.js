...
export function createExportImportHandlers(server: S3dbMCPServer) {
  return {
    async resourceExport(args: ResourceExportArgs, database: S3db): Promise<any> {
      ...
      try {
        ...
        switch (format) {
          ...
          case 'ndjson':
            exportData = data.map((doc: any) => JSON.stringify(doc)).join('\n');
    ...
    async resourceImport(args: ResourceImportArgs, database: S3db): Promise<any> {
      server.ensureConnected(database);
      ...
      try {
        ...
        for (let i = 0; i < data.length; i += batchSize) {
          ...
          switch (mode) {
            ...
            case 'upsert':
              batchResults = await Promise.all(batch.map((doc: any) => resource.upsert(doc)));
              break;
    ...
    async dbBackupMetadata(args: DbBackupMetadataArgs, database: S3db): Promise<any> {
      server.ensureConnected(database);
      ...
      try {
        ...
        const metadataContent = await (response.Body as any).transformToString();
        ...
        // Save backup
        await (database.client as any).putObject({
          Bucket: database.bucket,
          Key: backupKey,
