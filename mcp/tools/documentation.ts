...
const DOCUMENTATION_INDEX: DocIndex = {
  // Core concepts
...
interface DocSearchResult {
  file: string;
  score: number;
  sections: { header: string; content: string; score: number }[];
}
 ...
 */
function searchDocumentation(query: string): DocSearchResult[] {
 ...
 */
function extractRelevantSections(content: string, queryTerms: string[], filePath: string): { header: string; content: string; score: number }[] {
  const lines = content.split('\n');
  const sections: { header: string; content: string; score: number }[] = [];
 ...
 */
function formatResults(results: DocSearchResult[], query: string, maxResults: number = 5): FormattedResults {
  if (results.length === 0) {
    return {
      query,
      found: false,
  ...
  for (const result of topResults) {
    const formattedResult: FormattedDocResult = {
      file: result.file,
...
export function createDocumentationHandlers(server: S3dbMCPServer) {
  return {
    async s3dbQueryDocs(args: S3dbSearchDocsArgs): Promise<any> {
    ...
    async s3dbListTopics(args: S3dbListTopicsArgs): Promise<any> {
