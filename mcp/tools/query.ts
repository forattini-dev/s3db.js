...
export function createQueryHandlers(server: S3dbMCPServer) {
  return {
    async resourceQuery(args: ResourceQueryArgs, database: S3db): Promise<any> {
    ...
    async resourceSearch(args: { resourceName: string; searchText: string; fields?: string[]; caseSensitive?: boolean; limit?: number }, database: S3db): Promise<any> {
      server.ensureConnected(database);
