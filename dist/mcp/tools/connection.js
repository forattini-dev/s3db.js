...
export function createConnectionHandlers(server) {
    return {
        async dbConnect(args, database, injected) {
            const { connectionString, verbose = false, parallelism = 10, passphrase = 'secret', versioningEnabled = false, enableCache = true, enableCosts = true, cacheDriver = 'memory', cacheMaxSize = 1000, cacheTtl = 300000, cacheDirectory = './cache', cachePrefix = 'cache' } = args;
