...
import { BASE_USER_ATTRIBUTES, BASE_TENANT_ATTRIBUTES, BASE_CLIENT_ATTRIBUTES, mergeResourceConfig } from './concerns/resource-schemas.js';
...
export class IdentityPlugin extends Plugin {
    ...
    keyManager;
    oauth2KeysResource;
    oauth2AuthCodesResource;
    sessionsResource;
    passwordResetTokensResource;
    mfaDevicesResource;
    usersResource;
    tenantsResource;
    clientsResource;
    ...
    constructor(options = {}) {
        ...
        this.onboardingManager = new OnboardingManager({
            ...
            resources: {
                users: this.config.resources.users,
    ...
    async _createOAuth2Resources() {
        ...
        const [okKeys, errKeys, keysResource] = await tryFn(() => this.database.createResource({
        ...
        if (okKeys) {
            ...
            this.oauth2KeysResource = (this.database.resources[names.oauthKeys] ?? null);
            if (this.config.logLevel) {
        ...
        const [okCodes, errCodes, codesResource] = await tryFn(() => this.database.createResource({
        ...
        if (okCodes) {
            ...
            this.oauth2AuthCodesResource = (this.database.resources[names.authCodes] ?? null);
            if (this.config.logLevel) {
        ...
        const [okSessions, errSessions, sessionsResource] = await tryFn(() => this.database.createResource({
            name: names.sessions,
            attributes: {
                userId: 'string|required',
                expiresAt: 'string|required',
        ...
        if (okSessions) {
            ...
            this.sessionsResource = (this.database.resources[names.sessions] ?? null);
            if (this.config.logLevel) {
        ...
        const [okResetTokens, errResetTokens, resetTokensResource] = await tryFn(() => this.database.createResource({
        ...
        if (okResetTokens) {
            ...
            this.passwordResetTokensResource = (this.database.resources[names.passwordResetTokens] ?? null);
            if (this.config.logLevel) {
        ...
        if (this.config.mfa.enabled) {
            const [okMFA, errMFA, mfaResource] = await tryFn(() => this.database.createResource({
            ...
            if (okMFA) {
                ...
                this.mfaDevicesResource = (this.database.resources[names.mfaDevices] ?? null);
                if (this.config.logLevel) {
    ...
    async _createUserManagedResources() {
        ...
        const usersMergedConfig = mergeResourceConfig(usersBaseConfig, usersConfig.userConfig, 'users');
        usersConfig.mergedConfig = usersMergedConfig;
        const [okUsers, errUsers, usersResource] = await tryFn(() => this.database.createResource(usersMergedConfig));
        if (okUsers) {
            ...
            this.usersResource = (this.database.resources[usersMergedConfig.name] ?? null);
            if (this.config.logLevel) {
        ...
        const tenantsMergedConfig = mergeResourceConfig(tenantsBaseConfig, tenantsConfig.userConfig, 'tenants');
        tenantsConfig.mergedConfig = tenantsMergedConfig;
        const [okTenants, errTenants, tenantsResource] = await tryFn(() => this.database.createResource(tenantsMergedConfig));
        if (okTenants) {
            ...
            this.tenantsResource = (this.database.resources[tenantsMergedConfig.name] ?? null);
            if (this.config.logLevel) {
        ...
        const clientsMergedConfig = mergeResourceConfig(clientsBaseConfig, clientsConfig.userConfig, 'clients');
        clientsConfig.mergedConfig = clientsMergedConfig;
        const [okClients, errClients, clientsResource] = await tryFn(() => this.database.createResource(clientsMergedConfig));
        if (okClients) {
            ...
            this.clientsResource = (this.database.resources[clientsMergedConfig.name] ?? null);
            if (this.config.logLevel) {
    ...
    async _initializeOAuth2Server() {
        ...
        this.keyManager = this.oauth2Server.keyManager;
        if (this.config.logLevel) {
    ...
    _extractBuiltInDriverOptions(config) {
        ...
        const { drivers, custom, customDrivers, disableBuiltIns, ...builtInOptions } = config;
    ...
    async registerOAuthClient(options) {
        ...
        const { name, redirectUris = [], allowedScopes = this.config.supportedScopes, grantTypes = ['authorization_code', 'refresh_token'], responseTypes = ['code'], tokenEndpointAuthMethod = 'client_secret_basic', audiences = [], metadata = {} } = options;
