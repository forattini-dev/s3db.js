...
export class FailbanManager {
    ...
    async _createBansResource() {
        ...
        const ttlPlugin = this.database.pluginRegistry?.ttl ||
            this.database.pluginRegistry?.TTLPlugin;
        if (ttlPlugin && typeof ttlPlugin === 'object') {
            const pluginOptions = ttlPlugin.options;
            if (pluginOptions) {
    ...
    async _setupCleanupTimer() {
        ...
        this.cleanupTask = await cronManager.schedule('0 * * * * *', () => {
            const now = Date.now();
            let cleaned = 0;
    ...
    async _initializeGeoIP() {
        ...
        try {
            const geoip2 = await import('@maxmind/geoip2-node');
