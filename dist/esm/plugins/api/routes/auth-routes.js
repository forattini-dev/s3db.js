...
export function createAuthRoutes(authResource, config = {}, authMiddleware) {
    const app = new Hono();
    const { driver, drivers = [], usernameField = 'email', passwordField = 'password', jwtSecret, jwtExpiresIn = '7d', registration = {}, loginThrottle = {} } = config;
    if (authMiddleware) {
        app.use('/me', authMiddleware);
        app.use('/token/refresh', authMiddleware);
    }
    const hasExternalAuth = Array.isArray(drivers) && drivers.some(d => ['oidc', 'oauth2'].includes(d));
    ...
    if (registrationConfig.enabled) {
        app.post('/register', asyncHandler(async (c) => {
            ...
            for (const [key, value] of Object.entries(dataWithoutId)) {
                if (!allowedRegistrationFields.has(key))
                    continue;
                if (blockedRegistrationFields.has(key))
                    continue;
                if (key === usernameField || key === passwordField)
                    continue;
                userData[key] = value;
            }
            ...
            try {
                ...
                const response = formatter.error(err.message, { status: 500 });
                return c.json(response, response._status);
            }
        }));
    }
    ...
    if (jwtSecret) {
        app.post('/token/refresh', asyncHandler(async (c) => {
            ...
            const token = createToken({ userId: user.id, username: user.username, role: user.role }, jwtSecret, jwtExpiresIn);
            const response = formatter.success({
                token,
                expiresIn: jwtExpiresIn
            });
