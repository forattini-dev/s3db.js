...
export class SEOAnalyzer {
    ...
    _analyzeOnPageSEO(doc, html) {
        ...
        if (analysis.images.withoutAlt > 0) {
            analysis.recommendations.push(`${analysis.images.withoutAlt} images missing alt text - add descriptive alt text`);
        }
        ...
        analysis.contentMetrics = {
            ...
            quality: contentAnalysis.mainContentWordCount < 300
                ? 'short'
        ...
        if (contentAnalysis.mainContentWordCount < 300) {
            analysis.recommendations.push(`Content is too short (${contentAnalysis.mainContentWordCount} words) - aim for 300+ words for better SEO`);
        }
        if (contentAnalysis.contentRatio < 0.3) {
            analysis.recommendations.push(`Content ratio is low (${(contentAnalysis.contentRatio * 100).toFixed(1)}%) - too much navigation/boilerplate, focus on main content`);
        }
        return analysis;
    }
    /**
    ...
    _analyzeAccessibility(doc, html) {
        ...
        if (analysis.altText.withoutAlt > 0) {
            analysis.recommendations.push(`Add alt text to ${analysis.altText.withoutAlt} images for better accessibility`);
        }
        // Form labels
        ...
        analysis.formLabels = {
            ...
            issue: inputs.length > 0 && labels.length === 0
    ...
    _analyzeInternalLinks(doc, baseUrl) {
        ...
        const poorAnchorTexts = anchorTexts.filter((text) => text.toLowerCase() === 'click here' ||
        ...
        if (poorAnchorTexts.length > 0) {
            analysis.recommendations.push(`Use descriptive anchor text instead of "${poorAnchorTexts[0]}" - be specific about link content`);
        }
        // Check for broken anchors (orphaned content)
        const headingIds = Array.from(doc.querySelectorAll('[id]')).map((el) => el.id);
        const anchorLinks = Array.from(doc.querySelectorAll('a[href^="#"]')).map((link) => (link.getAttribute('href') || '').substring(1));
        ...
        if (externalWithoutRel > 0) {
            analysis.recommendations.push(`Add rel="noopener noreferrer" to ${externalWithoutRel} external links for security`);
        }
    ...
    _calculateSEOScore(result) {
        ...
        maxScore += 30;
        if (result.onPageSEO) {
            if (result.onPageSEO.h1?.count === 1)
                score += 10;
            if ((result.onPageSEO.headingStructure?.total || 0) > 0)
                score += 10;
            if ((result.onPageSEO.contentMetrics?.mainContentWordCount || 0) > 300)
                score += 10;
        }
        // Accessibility (20 points)
        maxScore += 20;
        if (result.accessibility) {
            if (result.accessibility.langAttribute?.present)
                score += 5;
            if ((result.accessibility.altText?.percentage || 0) > 80)
                score += 5;
            if (result.accessibility.headingStructure?.properlySorted)
                score += 5;
            if ((result.accessibility.semanticHTML?.score || 0) > 0)
                score += 5;
        }
        // Internal links (10 points)
        maxScore += 10;
        if (result.internalLinks) {
            if ((result.internalLinks.sameDomain?.count || 0) >= 5)
                score += 10;
        }
    ...
    _findHeadingIssues(headings) {
        ...
        if (h1Count !== 1)
            return `Found ${h1Count} H1 tags (should be 1)`;
        return null;
    }
