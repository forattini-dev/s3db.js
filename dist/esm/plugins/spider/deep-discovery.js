...
export class DeepDiscovery {
    ...
    async _detectPlatform(domain) {
        ...
        const traditionalCMS = detections.filter(d => d.type === 'cms' && ['wordpress', 'drupal', 'joomla'].includes(d.platform));
    ...
    _detectSitemapType(url) {
        ...
        if (urlLower.includes('index'))
            return 'sitemap-index';
        // Google News (time-sensitive, last 2 days)
        if (urlLower.includes('news') || urlLower.includes('gnews'))
            return 'google-news';
        // Google Images
    ...
    _detectFeedType(url, contentType) {
        if (url.includes('.json') || contentType?.includes('json'))
            return 'json';
        if (url.includes('atom') || contentType?.includes('atom'))
            return 'atom';
        if (url.includes('rss') || contentType?.includes('rss'))
            return 'rss';
        if (url.includes('mrss') || contentType?.includes('media-rss'))
            return 'mrss';
        return 'unknown';
    }
    /**
...
//# sourceMappingURL=deep-discovery.js.map