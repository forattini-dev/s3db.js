...
export class WebSocketServer extends EventEmitter {
    ...
    subscriptions; // resourceName -> Set<clientId>
    heartbeatTimers; // clientId -> { ping, timeout }
    rateLimitState; // clientId -> { count, windowStart }
    _resourceListeners;
    healthManager;
    channelManager;
    metrics;
    ...
    _matchesSubscriptionFilter(client, resourceName, data) {
        for (const subKey of client.subscriptions) {
            if (!subKey.startsWith(`${resourceName}:`))
                continue;
    ...
    _filterProtectedFields(data, resourceConfig) {
        if (!data || !resourceConfig?.protected)
            return data;
        const result = { ...data };
    ...
    getInfo() {
        return {
            ...
            subscriptions: Object.fromEntries(Array.from(this.subscriptions.entries()).map(([k, v]) => [k, v.size])),
