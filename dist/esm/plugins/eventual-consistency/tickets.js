...
export async function claimTickets(ticketResource, workerId, config) {
    ...
    const availableTickets = await ticketResource.query({ status: 'available' }, { limit: claimLimit * 2 });
...
export async function processTicket(ticket, handler, database) {
    ...
    for (const originalId of ticket.records) {
        try {
            const transactions = await transactionResource.query({ originalId, applied: false }, { limit: Infinity });
