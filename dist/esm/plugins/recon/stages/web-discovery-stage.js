...
export class WebDiscoveryStage {
    ...
    async execute(target, featureConfig = {}) {
        ...
        const result = await this.commandRunner.runRedBlue('web', 'asset', 'fuzz', url, {
    ...
    _normalizeDiscovery(data) {
        ...
        if (Array.isArray(data.paths)) {
            paths.push(...data.paths.map((p) => this._normalizePath(p)).filter(Boolean));
        }
        else if (Array.isArray(data.results)) {
            paths.push(...data.results.map((p) => this._normalizePath(p)).filter(Boolean));
        }
        else if (Array.isArray(data)) {
            paths.push(...data.map((p) => this._normalizePath(p)).filter((p) => p !== null));
        }
        return {
...
//# sourceMappingURL=web-discovery-stage.js.map