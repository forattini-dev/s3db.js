...
export class ReplicatorPlugin extends Plugin {
    ...
    resourceFilter;
    replicators = [];
    eventListenersInstalled = new Set();
    ...
    constructor(options = {}) {
        ...
        const typedOptions = this.options;
        const { replicators = [], resourceNames = {}, replicatorConcurrency, stopConcurrency, logErrors = true, persistReplicatorLog = false, enabled = true, batchSize = 100, maxRetries = 3, timeout = 30000, replicatorLogResource } = typedOptions;
    ...
    async getCompleteData(resource, data) {
        ...
        this.logger.warn({ resource: resource?.name || 'unknown', id: data?.id, reason }, `Falling back to provided data for ${resource?.name || 'unknown'}#${data?.id}: ${reason}`);
        return data;
    }
    installEventListeners(resource, database, plugin) {
        ...
        const insertHandler = async (data) => {
            ...
            if (!ok) {
                this.logger.warn({ resource: resource.name, operation: 'insert', error: error.message }, `Insert event failed for resource ${resource.name}: ${error.message}`);
        ...
        const updateHandler = async (data, beforeData) => {
            ...
            if (!ok) {
                this.logger.warn({ resource: resource.name, operation: 'update', error: error.message }, `Update event failed for resource ${resource.name}: ${error.message}`);
        ...
        const deleteHandler = async (data) => {
            ...
            if (!ok) {
                this.logger.warn({ resource: resource.name, operation: 'delete', error: error.message }, `Delete event failed for resource ${resource.name}: ${error.message}`);
    ...
    installDatabaseHooks() {
        ...
        this._afterCreateResourceHook = (resource) => {
            ...
            if (res.name !== this.logResourceName && this._shouldManageResource(res.name)) {
                this.installEventListeners(res, this.database, this);
            }
        };
    ...
    async retryWithBackoff(operation, maxRetries = 3) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            if (ok) {
                ...
                lastError = error;
                this.logger.warn({ attempt, maxRetries, error: error.message }, `Retry attempt ${attempt}/${maxRetries} failed: ${error.message}`);
    ...
    async logError(replicator, resourceName, operation, recordId, data, error) {
        ...
        if (!ok) {
            this.logger.warn({ replicator: replicator.name || replicator.id, resourceName, error: logError.message }, `Failed to log error for ${resourceName}: ${logError.message}`);
    ...
    async processReplicatorEvent(operation, resourceName, recordId, data, beforeData = null) {
        ...
        const poolResult = await PromisePool
            ...
            this.logger.warn({ replicator: replicator.name || replicator.id, resourceName, operation, error: error.message }, `Replication failed for ${replicator.name || replicator.id} on ${resourceName}: ${error.message}`);
    ...
    async processReplicatorItem(item) {
        ...
        await PromisePool
            ...
            const [wrapperOk, wrapperError] = await tryFn(async () => {
                ...
                const [ok, err, result] = await tryFn(() => replicator.replicate(item.resourceName, item.operation, preparedData, item.recordId, preparedBefore));
                if (!ok) {
                    this.logger.warn({ replicator: replicator.name || replicator.id, resourceName: item.resourceName, operation: item.operation, error: err.message }, `Replicator item processing failed for ${replicator.name || replicator.id} on ${item.resourceName}: ${err.message}`);
                    this.emit('plg:replicator:error', {
                        replicator: replicator.name || replicator.id,
                    ...
                    if (this.config.logErrors && this.database) {
                        await this.logError(replicator, item.resourceName, item.operation, item.recordId, item.data || null, err);
                    }
                    this.stats.totalErrors += 1;
            ...
            this.logger.warn({ replicator: replicator.name || replicator.id, resourceName: item.resourceName, operation: item.operation, error: wrapperError.message }, `Wrapper processing failed for ${replicator.name || replicator.id} on ${item.resourceName}: ${wrapperError.message}`);
            this.emit('plg:replicator:error', {
                replicator: replicator.name || replicator.id,
                resourceName: item.resourceName,
                operation: item.operation,
            ...
            if (this.config.logErrors && this.database) {
                await this.logError(replicator, item.resourceName, item.operation, item.recordId, item.data || null, wrapperError);
            }
            this.stats.totalErrors += 1;
    ...
    async getReplicatorLogs(options = {}) {
        ...
        const { resourceName, operation, status, limit = 100, offset = 0 } = options;
        const filter = {};
        if (resourceName) {
    ...
    async retryFailedReplicators() {
        ...
        const processResult = await PromisePool
            ...
            const [ok, err, results] = await tryFn(async () => {
                return await this.processReplicatorEvent(log.operation, log.resourceName, log.recordId, sanitizedData, sanitizedBefore);
            });
    ...
    async syncAllData(replicatorId) {
        ...
        for (const resourceName in this.database.resources) {
            if (resourceName === this.logResourceName)
                continue;
            if (replicator.shouldReplicateResource(resourceName)) {
                ...
                while (true) {
                    ...
                    const poolResult = await PromisePool
                        ...
                        if (!replicateOk) {
                            this.logger.warn({ replicator: replicator.name || replicator.id, resourceName, error: replicateError.message }, `syncAllData failed for ${replicator.name || replicator.id} on ${resourceName}: ${replicateError.message}`);
                            this.stats.totalErrors += 1;
                            this.emit('plg:replicator:error', {
                                replicator: replicator.name || replicator.id,
                                resourceName,
        ...
        const [ok, error] = await tryFn(async () => {
            if (this.replicators && this.replicators.length > 0) {
                await PromisePool
                    ...
                    if (!replicatorOk) {
                        this.logger.warn({ replicator: replicator.name || replicator.id, driver: replicator.driver, error: replicatorError.message }, `Failed to stop replicator ${replicator.name || replicator.id}: ${replicatorError.message}`);
