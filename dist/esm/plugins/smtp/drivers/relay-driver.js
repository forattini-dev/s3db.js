...
const PROVIDER_CONFIGS = {
    ...
    'gmail': {
        ...
        auth: {
            ...
            pass: (config) => config.appPassword || ''
        }
    }
};
export class SMTPRelayDriver {
    ...
    async initialize() {
        ...
        try {
            ...
            if (!verified) {
                throw new ConnectionError(`Failed to verify SMTP connection for driver "${this.name}"`, {
                    driver: this.name,
            ...
            throw new ConnectionError(`Failed to initialize SMTP driver "${this.name}": ${err.message}`, {
                originalError: err,
                driver: this.name
            });
        }
    ...
    _buildSmtpConfig() {
        ...
        if (PROVIDER_CONFIGS[this.name]) {
            ...
            if (this.config.port)
                smtpConfig.port = this.config.port;
            if (this.config.secure !== undefined)
                smtpConfig.secure = this.config.secure;
        ...
        if (!smtpConfig.auth.user || !smtpConfig.auth.pass) {
            throw new AuthenticationError(`SMTP driver "${this.name}" requires authentication credentials`, {
                driver: this.name,
    ...
    async sendEmail(emailData) {
        ...
        try {
            ...
            throw new ConnectionError(`Failed to send email via ${this.name}: ${err.message}`, {
                driver: this.name,
                originalError: err
            });
        }
...
//# sourceMappingURL=relay-driver.js.map