...
function isVersionCompatible(actual, required) {
    ...
    const actualMajor = parseInt(actual.split('.')[0] ?? '0', 10);
    const requiredMajor = parseInt(cleanRequired.split('.')[0] ?? '0', 10);
...
async function tryLoadPackage(packageName) {
    try {
        ...
        return { installed: false, version: null, error: error };
    }
}
export async function requirePluginDependency(pluginId, optionsOrCallerName = {}) {
    const options = typeof optionsOrCallerName === 'string'
        ? { callerName: optionsOrCallerName }
        : optionsOrCallerName;
    const { throwOnError = true, checkVersions = true } = options;
    const pluginDef = PLUGIN_DEPENDENCIES[pluginId];
    if (!pluginDef) {
        const error = new Error(`Unknown plugin identifier: ${pluginId}. ` +
    ...
    for (const [pkgName, pkgInfo] of Object.entries(pluginDef.dependencies)) {
        const { installed, version } = await tryLoadPackage(pkgName);
        if (!installed) {
            missing.push(pkgName);
            messages.push(`❌ Missing dependency: ${pkgName}\n` +
        ...
        if (checkVersions && version && version !== 'unknown') {
            ...
            if (!compatible) {
                ...
                messages.push(`⚠️  Incompatible version: ${pkgName}\n` +
                    `   Installed: ${version}\n` +
                    `   Required: ${pkgInfo.version}\n` +
                    `   Update: ${pkgInfo.installCommand}`);
            }
            else {
                messages.push(`✅ ${pkgName}@${version} (compatible with ${pkgInfo.version})`);
            }
        }
        else {
            messages.push(`✅ ${pkgName}@${version || 'unknown'} (installed)`);
        }
    }
...
//# sourceMappingURL=plugin-dependencies.js.map