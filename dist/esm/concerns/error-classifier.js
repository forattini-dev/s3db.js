...
export const NON_RETRIABLE = 'NON_RETRIABLE';
...
export class ErrorClassifier {
    static classify(error, options = {}) {
        ...
        const { retryableErrors = [], nonRetriableErrors = [] } = options;
        if (retryableErrors.length > 0) {
            const isCustomRetriable = retryableErrors.some(errType => error.code === errType ||
                error.name === errType ||
                error.message?.includes(errType));
            if (isCustomRetriable)
                return RETRIABLE;
        }
        if (nonRetriableErrors.length > 0) {
            const isCustomNonRetriable = nonRetriableErrors.some(errType => error.code === errType ||
                error.name === errType ||
                error.message?.includes(errType));
            if (isCustomNonRetriable)
                return NON_RETRIABLE;
        }
