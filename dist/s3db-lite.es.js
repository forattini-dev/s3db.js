...
import { expandHTTP2Options, createHttp2MetricsHooks, createClient, getGlobalHttp2Metrics, parseHttp2Error, Http2Error } from 'recker';
...
import os, { cpus, platform } from 'os';
import { setTimeout as setTimeout$1 } from 'timers/promises';
import { nanoid } from 'nanoid';
import { writeFile, readFile, mkdir, rename, unlink, appendFile, copyFile, readdir, stat } from 'fs/promises';
...
class TasksPool extends EventEmitter {
    ...
    _sleep(ms, signal) {
        if (signal && typeof signal.aborted !== 'undefined') {
            return setTimeout$1(ms, undefined, { signal });
        }
        return setTimeout$1(ms);
    }
...
class S3Client extends EventEmitter$1 {
    id;
    logLevel;
    logger;
    config;
...
class TasksRunner extends EventEmitter {
...
class MemoryClient extends EventEmitter$1 {
    id;
    logLevel;
    logger;
...
class FileSystemClient extends EventEmitter$1 {
    id;
    logLevel;
    logger;
    taskExecutorMonitoring;
...
class SafeEventEmitter extends EventEmitter$1 {
    options;
    logger;
    _signalHandlersSetup;
    _isDestroyed;
...
class ResourceIdsReader extends EventEmitter$1 {
    resource;
    client;
    stream;
    controller;
...
class ResourceReader extends EventEmitter$1 {
    resource;
    client;
    batchSize;
    concurrency;
...
class ResourceWriter extends EventEmitter$1 {
    resource;
    client;
    batchSize;
    concurrency;
    ...
    async startPlugins() {
        ...
        if (!isEmpty(db.pluginList)) {
            ...
            for (const p of db.pluginList) {
                try {
                    const plugin = isFunction(p) ? new p(db) : p;
                    plugins.push(plugin);
                }
                catch (error) {
...
class GlobalCoordinatorService extends EventEmitter {
    namespace;
    database;
    serviceId;
    workerId;
...
//# sourceMappingURL=s3db-lite.es.js.map
