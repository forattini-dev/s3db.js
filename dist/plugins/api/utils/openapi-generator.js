...
export function generateResourcePaths(resource, version, config = {}) {
    ...
    if (hasPartitions) {
        ...
        const pluginAttrNames = resource.schema?._pluginAttributes
            ? Object.values(resource.schema._pluginAttributes).flat()
            : [];
        const filteredAttributes = Object.fromEntries(Object.entries(allAttributes).filter(([name]) => !pluginAttrNames.includes(name)));
...
function generateRelationalPaths(resource, relationName, relationConfig, version, relatedSchema, versionPrefix = '', basePathPrefix = '', resolveSecurityForPath = null) {
    const resourceName = resource.name;
    const resourceLabel = formatResourceLabel(resourceName);
    const relationLabel = formatResourceLabel(relationName);
    const basePath = applyBasePath(basePathPrefix, versionPrefix
...
export function generateOpenAPISpec(database, config = {}) {
    const { title = 's3db.js API', version = '1.0.0', description = 'Auto-generated REST API documentation for s3db.js resources', serverUrl = 'http://localhost:3000', auth = {}, resources: resourceConfigs = {}, versionPrefix: globalVersionPrefix, basePath = '', routes: pluginRoutes = {}, app = null } = config;
...
For detailed information about each endpoint, see the sections below.`;
    ...
    const registerCustomRouteOperation = (params) => {
    ...
    for (const [name, resource] of Object.entries(resources)) {
        ...
        const resourceBaseMountPath = applyBasePath(normalizedBasePath, prefix ? `/${prefix}/${name}` : `/${name}`);
        ...
        if (relationsPlugin && relationsPlugin.relations && relationsPlugin.relations[name]) {
            ...
            for (const [relationName, relationConfig] of Object.entries(relationsDef)) {
                ...
                const relationalPaths = generateRelationalPaths(resource, relationName, relationConfig, resourceVersion, relatedSchema, prefix, normalizedBasePath, resolveSecurityForPath);
