...
export function createFilesystemHandler(config) {
    const { root, index = ['index.html'], fallback = false, maxAge = 0, dotfiles = 'ignore', etag = true, cors = false } = config;
    if (!root) {
    ...
    return async (c) => {
        try {
            ...
            if (logLevel === 'debug' || logLevel === 'trace') {
                logger.error({ error: err.message }, '[Static Filesystem] Error');
            }
...
//# sourceMappingURL=static-filesystem.js.map