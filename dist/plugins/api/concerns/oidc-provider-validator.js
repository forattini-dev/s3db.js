...
export function validateProviderCompatibility(discoveryDoc, config) {
    const warnings = [];
    const errors = [];
    if (!discoveryDoc) {
    ...
    if (supportedAlgs.length > 0 && !supportedAlgs.includes(requestedAlg)) {
        warnings.push(`ID token signing algorithm "${requestedAlg}" not listed in provider's ` +
    ...
    if (supportedTypes.length > 0 && !supportedTypes.includes(requestedType)) {
        warnings.push(`Response type "${requestedType}" not listed in provider's ` +
    ...
    if (requestedMode && supportedModes.length > 0 && !supportedModes.includes(requestedMode)) {
        warnings.push(`Response mode "${requestedMode}" not listed in provider's ` +
    ...
    if (supportedScopes.length > 0) {
        const unsupportedScopes = requestedScopes.filter(scope => !supportedScopes.includes(scope));
        if (unsupportedScopes.length > 0) {
            warnings.push(`Requested scopes not listed in provider's supported scopes: ` +
    ...
    if (config.autoRefreshTokens && supportedGrants.length > 0) {
        if (!supportedGrants.includes('refresh_token')) {
            warnings.push('autoRefreshTokens enabled but provider does not list "refresh_token" ' +
                'in supported grant types. Token refresh will likely fail.');
        }
    }
    if (!discoveryDoc.token_endpoint && requestedType === 'code') {
        errors.push('Provider discovery document missing "token_endpoint" but response_type is "code". ' +
            'Token exchange will fail.');
    }
    if (!discoveryDoc.authorization_endpoint) {
        errors.push('Provider discovery document missing "authorization_endpoint". ' +
    ...
    if (config.usePKCE && supportedCodeChallengeMethods.length > 0) {
        if (!supportedCodeChallengeMethods.includes('S256')) {
            warnings.push('PKCE enabled but provider does not list "S256" in supported code challenge methods. ' +
                'PKCE may not work correctly.');
        }
    }
    if (config.enableLogout && !discoveryDoc.end_session_endpoint) {
        warnings.push('Logout enabled but provider does not provide "end_session_endpoint". ' +
            'Only local logout will be available (session will remain active at provider).');
    }
    if (!discoveryDoc.userinfo_endpoint) {
        warnings.push('Provider does not provide "userinfo_endpoint". ' +
    ...
    if (config.autoRefreshTokens) {
        ...
        if (!hasOfflineAccess && !hasRefreshToken) {
            warnings.push('autoRefreshTokens enabled but neither "offline_access" nor "refresh_token" ' +
    ...
    if (supportedAuthMethods.length > 0 && !supportedAuthMethods.includes(requestedAuthMethod)) {
        warnings.push(`Token endpoint auth method "${requestedAuthMethod}" not listed in provider's ` +
    ...
    if (supportedClaims.length > 0) {
        ...
        const missingClaims = essentialClaims.filter(claim => !supportedClaims.includes(claim));
        if (missingClaims.length > 0) {
            warnings.push(`Provider does not list essential claims in supported claims: ` +
...
export function logProviderCompatibility(result, options = {}) {
    ...
    if (result.errors.length > 0) {
        ...
        if (throwOnError) {
            throw new Error(`Provider compatibility errors detected:\n${result.errors.join('\n')}`);
        }
    }
...
export function getProviderCapabilities(discoveryDoc) {
    if (!discoveryDoc) {
        return {
...
//# sourceMappingURL=oidc-provider-validator.js.map