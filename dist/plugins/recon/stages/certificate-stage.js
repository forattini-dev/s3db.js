...
export class CertificateStage {
    ...
    async execute(target, featureConfig = {}) {
        const shouldCheckTls = target.protocol === 'https' ||
        ...
        const result = await this.commandRunner.runRedBlue('web', 'asset', 'cert', hostPort, {
    ...
    _normalizeAltNames(altNames) {
        ...
        if (Array.isArray(altNames)) {
            return altNames.map(name => typeof name === 'string' ? name.trim() : String(name));
        }
...
//# sourceMappingURL=certificate-stage.js.map