...
import { GraphConfigurationError, PathNotFoundError } from './graph.errors.js';
export class GraphPlugin extends Plugin {
    config;
    _resourceGraphNamespaces;
    constructor(options = {}) {
        super(options);
        if (options.logger) {
        ...
        const { vertices = null, edges = null, directed = true, weighted = false, defaultWeight = 1, maxTraversalDepth = 50, createResources = false, vertexIdField = 'id', edgeSourceField = 'source', edgeTargetField = 'target', edgeLabelField = 'label', edgeWeightField = 'weight', denormalize = [] } = opts;
        this.config = {
            ...
            vertexIdField: vertexIdField,
            edgeSourceField: edgeSourceField,
            edgeTargetField: edgeTargetField,
            edgeLabelField: edgeLabelField,
            edgeWeightField: edgeWeightField,
    ...
    _installResourceMethods() {
        for (const resource of Object.values(this.database.resources)) {
            ...
            if (!isVertex && !isEdge)
                continue;
            if (this._resourceGraphNamespaces.has(resource.name))
                continue;
    ...
    async _getNeighbors(vertexResourceName, vertexId, options = {}) {
        ...
        if (neighborIdArray.length > 0) {
            const resourceWithGetMany = vertexResource;
    ...
    async _findShortestPath(vertexResourceName, fromId, toId, options = {}) {
        const { maxDepth = this.config.maxTraversalDepth, heuristic = null, returnPath = true, direction = 'outgoing', includeStats = false } = options;
        if (fromId === toId) {
            const result = { path: [fromId], distance: 0, edges: [] };
            if (includeStats)
                result.stats = { iterations: 0, visited: 0 };
            return result;
        }
        ...
        while (!openSet.isEmpty() && iterations < maxIterations) {
            ...
            if (currentId === toId) {
                if (!returnPath) {
                    ...
                    if (includeStats)
                        result.stats = { iterations, visited: visited.size };
                    return result;
                }
    ...
    async _traverse(vertexResourceName, startId, options = {}) {
        const { maxDepth = this.config.maxTraversalDepth, direction = 'outgoing', filter = null, visitor = null, mode = 'bfs' } = options;
        const visited = new Set();
        const result = [];
...
//# sourceMappingURL=graph.plugin.js.map