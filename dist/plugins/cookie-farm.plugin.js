...
export class CookieFarmPlugin extends Plugin {
    ...
    puppeteerPlugin;
    stealthManager; // Placeholder for StealthManager
    personaPool; // personaId -> persona object
    initialized;
    constructor(options = {}) {
        super(options);
    ...
    _findPuppeteerDependency() {
        const registries = [
            ...
            this.database?.pluginList // Assuming pluginList could exist
        ].filter(Boolean);
        const candidates = [];
        ...
        const sameNamespace = candidates.find(plugin => plugin instanceof PuppeteerPlugin &&
    ...
    async generatePersonas(count = 1, options = {}) {
        const { proxies = this.config.generation.proxies, warmup = this.config.warmup.enabled } = options;
    ...
    async _visitSite(persona, url) {
        ...
        if (this.config.warmup?.interactions?.scroll) {
            ...
            if (page.humanScroll) { // Check if method exists before calling
    ...
    async _calculateQuality(persona) {
        ...
        const requestScore = Math.min(persona.reputation.totalRequests / (this.config.rotation?.maxRequests ?? 1), 1);
    ...
    async getPersona(criteria = {}) {
        const { quality = null, // 'low' | 'medium' | 'high'
        minQualityScore = 0, proxyId = null, excludeRetired = true } = criteria;
...
//# sourceMappingURL=cookie-farm.plugin.js.map