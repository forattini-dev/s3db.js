...
export class NeuralNetworkModel extends BaseModel {
    ...
    _validateLayersConfig() {
        if (!Array.isArray(this.config.modelConfig.layers) || this.config.modelConfig.layers.length === 0) {
            throw new ModelConfigError('Neural network must have at least one hidden layer', { model: this.config.name, layers: this.config.modelConfig.layers });
        }
        for (const [index, layer] of this.config.modelConfig.layers.entries()) {
            if (!layer.units || typeof layer.units !== 'number' || layer.units < 1) {
                throw new ModelConfigError(`Layer ${index} must have a valid "units" property (positive number)`, { model: this.config.name, layer, index });
            }
            if (layer.activation && !this._isValidActivation(layer.activation)) {
                throw new ModelConfigError(`Layer ${index} has invalid activation function "${layer.activation}"`, { model: this.config.name, layer, index, validActivations: ['relu', 'sigmoid', 'tanh', 'softmax', 'elu', 'selu'] });
            }
        }
    }
    /**
    ...
    addLayer(layerConfig) {
        if (this.model) {
            throw new ModelConfigError('Cannot add layer after model is built. Use addLayer() before training.', { model: this.config.name });
        }
    ...
    setOutput(outputConfig) {
        if (this.model) {
            throw new ModelConfigError('Cannot change output after model is built. Use setOutput() before training.', { model: this.config.name });
        }
    ...
    async trainWithEarlyStopping(data, earlyStoppingConfig = {}) {
        const { patience = 10, minDelta = 0.001, monitor = 'val_loss', restoreBestWeights = true } = earlyStoppingConfig;
        // Prepare data
...
//# sourceMappingURL=neural-network-model.class.js.map