...
export class ReckerLinkDiscoverer {
    ...
    parse = null;
    reckerExtractLinks = null;
    fallbackDiscoverer = null;
    constructor(config = {}) {
        this.config = {
    ...
    async _checkReckerAvailability() {
        ...
        try {
            ...
            this.parse = parserModule.parse;
            this.reckerExtractLinks = extractorsModule.extractLinks;
            ...
            try {
                ...
                if (recker.parseHtmlSync && recker.extractLinks) {
                    this.parse = recker.parseHtmlSync;
                    this.reckerExtractLinks = recker.extractLinks;
                    this.reckerAvailable = true;
                    return true;
                }
            }
    ...
    extractLinksSync(html, baseUrl, currentDepth = 0) {
    ...
    _extractLinksWithRecker(html, baseUrl, currentDepth) {
        const links = [];
        ...
        try {
            ...
            for (const item of extracted) {
                if (!item.href || item.href.trim() === '')
                    continue;
                if (item.type === 'anchor' || item.type === 'mailto' || item.type === 'tel')
                    continue;
                try {
    ...
    _extractLinksWithRegex(html, baseUrl, currentDepth) {
        const links = [];
        const baseUrlObj = new URL(baseUrl);
    ...
    getStats() {
        return {
            ...
            sitemapStats: (this.sitemapValidator?.getStats() || null)
        };
    }
    reset(options = {}) {
    ...
    async getRobotsValidation(url) {
        if (!this.robotsValidator)
            return null;
        return this.robotsValidator.validate(url);
    }
    async getSitemapValidation(url) {
        if (!this.sitemapValidator)
            return null;
        try {
            const urlObj = new URL(url);
...
//# sourceMappingURL=recker-link-discoverer.js.map