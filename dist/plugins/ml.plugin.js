...
import { MLError, ModelConfigError, ModelNotFoundError, TrainingError, TensorFlowDependencyError } from './ml.errors.js'; // Import from TS version
export class MLPlugin extends Plugin {
    config;
    models;
    _dependenciesValidated;
    modelVersions;
    modelCache;
    training;
    insertCounters;
    _pendingAutoTrainingHandlers;
    _autoTrainingInitialized;
    cronManager; // CronManager type not explicitly defined
    jobNames;
    stats;
    constructor(options = {}) {
        super(options);
        ...
        const { models = {}, minTrainingSamples = 10, saveModel = true, saveTrainingData = false, enableVersioning = true, ...rest } = options;
        this.config = {
            models: models,
    ...
    async onInstall() {
        ...
        if (!this._dependenciesValidated) {
            ...
            try {
                ...
                if (!result.valid) {
                    throw new TensorFlowDependencyError('TensorFlow.js dependency not found. Install with: pnpm add @tensorflow/tfjs-node\n' +
        ...
        this.logger.debug({ modelCount: Object.keys(this.models).length }, `Installed with ${Object.keys(this.models).length} models`);
    ...
    _buildModelCache() {
        for (const [modelName, modelConfig] of Object.entries(this.config.models)) {
            ...
            this.logger.debug({ modelName, resource: modelConfig.resource, target: modelConfig.target }, `Cached model "${modelName}" for ${modelConfig.resource}.predict(..., '${modelConfig.target}')`);
        }
    }
    /**
     * Inject ML methods into Resource prototype
                ...
                get() {
                    ...
                    return {
                        ...
                        versions: async (target) => {
                            ...
                            if (!modelName) {
                                throw new ModelNotFoundError(`No model found for resource "${resource.name}" with target "${target}"`, { resourceName: resource.name, targetAttribute: target });
                            }
    ...
    async _resourceLearn(resourceName, target, options = {}) {
        ...
        this.logger.debug({ modelName, resource: resourceName, target }, `Auto-creating model "${modelName}" for ${resourceName}.${target}...`);
        // Get resource
        const resource = this.database.resources[resourceName];
        if (!resource) {
            throw new ModelConfigError(`Resource "${resourceName}" not found`, { resourceName, availableResources: Object.keys(this.database.resources) });
        }
        ...
        if (!userProvidedBatchSize && sampleCount > 0 && sampleCount < defaultModelConfig.batchSize) {
            ...
            this.logger.debug({ batchSize: defaultModelConfig.batchSize, sampleCount }, `Auto-adjusted batchSize to ${defaultModelConfig.batchSize} based on ${sampleCount} samples`);
        }
    ...
    async _autoSelectFeatures(resourceName, target) {
        ...
        for (const [fieldName, fieldDef] of Object.entries(attributes)) {
            ...
            if (fieldName === target)
                continue;
            // Skip system fields
            if (['id', 'createdAt', 'updatedAt', 'createdBy'].includes(fieldName))
                continue;
        ...
        if (numericFields.length === 0) {
            throw new ModelConfigError(`No numeric features found for target "${target}" in resource "${resourceName}"`, { resourceName, target, availableAttributes: Object.keys(attributes) });
        }
    ...
    async _resourcePredict(resourceName, input, targetAttribute) {
        ...
        if (!modelName) {
            throw new ModelNotFoundError(`No model found for resource "${resourceName}" with target "${targetAttribute}"`, { resourceName, targetAttribute, availableModels: Object.keys(this.models) });
        }
        this.logger.debug({ resourceName, targetAttribute, modelName }, `Resource prediction: ${resourceName}.predict(..., '${targetAttribute}') -> model "${modelName}"`);
    ...
    async _resourceTrainModel(resourceName, targetAttribute, options = {}) {
        ...
        this.logger.debug({ resourceName, targetAttribute, modelName }, `Resource training: ${resourceName}.trainModel('${targetAttribute}') -> model "${modelName}"`);
        return await this.train(modelName, options);
    }
    /**
    ...
    _validateModelConfig(modelName, config) {
        ...
        if (!config.type || !validTypes.includes(config.type)) {
            throw new ModelConfigError(`Model "${modelName}" must have a valid type: ${validTypes.join(', ')}`, { modelName, type: config.type, validTypes });
        }
        if (!config.resource) {
            throw new ModelConfigError(`Model "${modelName}" must specify a resource`, { modelName });
        }
        if (!config.features || !Array.isArray(config.features) || config.features.length === 0) {
            throw new ModelConfigError(`Model "${modelName}" must specify at least one feature`, { modelName, features: config.features });
        }
        if (!config.target) {
            throw new ModelConfigError(`Model "${modelName}" must specify a target field`, { modelName });
        }
    }
    /**
        ...
        try {
            switch (config.type) {
                ...
                case 'neural-network':
                    ...
                    break;
                default:
                    throw new ModelConfigError(`Unknown model type: ${config.type}`, { modelName, type: config.type });
            }
    ...
    */
    _setupAutoTraining(modelName, config) {
        ...
        if (!resource) {
            this.logger.warn({ resource: config.resource, modelName }, `Resource "${config.resource}" not found for model "${modelName}". Auto-training will attach when resource is created.`);
        ...
        if (config.trainAfterInserts && config.trainAfterInserts > 0) {
            this.addMiddleware(resource, 'insert', async (next, data, options) => {
                ...
                if (this.insertCounters.get(modelName) >= config.trainAfterInserts) {
                    this.logger.debug({ modelName, insertCount: config.trainAfterInserts }, `Auto-training "${modelName}" after ${config.trainAfterInserts} inserts`);
                    // Reset counter
                    this.insertCounters.set(modelName, 0);
        ...
        if (config.trainInterval && config.trainInterval > 0) {
            ...
            this.cronManager.scheduleInterval(config.trainInterval, async () => {
                this.logger.debug({ modelName, trainInterval: config.trainInterval }, `Auto-training "${modelName}" (interval: ${config.trainInterval}ms)`);
                try {
            ...
            this.logger.debug({ modelName, trainInterval: config.trainInterval }, `Setup interval training for "${modelName}" (every ${config.trainInterval}ms)`);
        }
    ...
    async train(modelName, options = {}) {
        ...
        if (!model) {
            throw new ModelNotFoundError(`Model "${modelName}" not found`, { modelName, availableModels: Object.keys(this.models) });
        }
        ...
        try {
            ...
            if (!modelConfig) {
                throw new ModelNotFoundError(`Model "${modelName}" not found in configuration`, { modelName });
            }
            // Get resource
            const resource = this.database.resources[modelConfig.resource];
            if (!resource) {
                throw new ModelNotFoundError(`Resource "${modelConfig.resource}" not found`, { modelName, resource: modelConfig.resource });
            }
            ...
            if (partition && partition.name) {
                ...
                this.logger.debug({ modelName, partition: partition.name, partitionValues: partition.values }, `Using partition "${partition.name}" with values: ${JSON.stringify(partition.values)}`);
                const [ok, err, partitionData] = await tryFn(() => resource.listPartition({
                    partition: partition.name,
                    partitionValues: partition.values
                }));
                if (!ok) {
                    throw new TrainingError(`Failed to fetch training data from partition: ${err.message}`, { modelName, resource: modelConfig.resource, partition: partition.name, originalError: err.message });
                }
                data = partitionData;
            }
            else {
                ...
                if (!ok) {
                    throw new TrainingError(`Failed to fetch training data: ${err.message}`, { modelName, resource: modelConfig.resource, originalError: err.message });
                }
                data = allData;
            }
            ...
            if (modelConfig.filter && typeof modelConfig.filter === 'function') {
                ...
                this.logger.debug({ modelName, originalLength, filteredLength: data.length }, `Filter reduced dataset from ${originalLength} to ${data.length} samples`);
            }
            ...
            if (!data || data.length < this.config.minTrainingSamples) {
                throw new TrainingError(`Insufficient training data: ${data?.length || 0} samples (minimum: ${this.config.minTrainingSamples})`, { modelName, samples: data?.length || 0, minimum: this.config.minTrainingSamples });
            }
            this.logger.debug({ modelName, sampleCount: data.length }, `Training "${modelName}" with ${data.length} samples...`);
            ...
            this.logger.debug({ modelName, result }, `Training completed for "${modelName}": ${JSON.stringify(result)}`);
            ...
            throw new TrainingError(`Training failed for "${modelName}": ${error.message}`, { modelName, originalError: error.message });
        }
        finally {
    ...
    async _initializeVersioning(modelName) {
        try {
            ...
            const [ok, err, versionInfo] = await tryFn(() => storage.get(storage.getPluginKey(resourceName, 'metadata', modelName, 'versions')));
            if (ok && versionInfo) {
                ...
                this.logger.debug({ modelName, version: versionInfo.currentVersion }, `Loaded version info for "${modelName}": v${versionInfo.currentVersion}`);
            }
            else {
    ...
    async _updateVersionInfo(modelName, version) {
        try {
            ...
            await storage.set(storage.getPluginKey(resourceName, 'metadata', modelName, 'versions'), {
                modelName,
                currentVersion: versionInfo.currentVersion,
                latestVersion: versionInfo.latestVersion,
                updatedAt: new Date().toISOString()
            }, { behavior: 'body-overflow' });
            this.logger.debug({ modelName, currentVersion: versionInfo.currentVersion, latestVersion: versionInfo.latestVersion }, `Updated version info for "${modelName}": current=v${versionInfo.currentVersion}, latest=v${versionInfo.latestVersion}`);
        }
        catch (error) {
    ...
    async _saveModel(modelName) {
        try {
            ...
            if (enableVersioning) {
                ...
                await storage.set(storage.getPluginKey(resourceName, 'models', modelName, `v${version}`), {
                    modelName,
                    version,
                    type: 'model',
                ...
                this.logger.debug({ modelName, version, resourceName }, `Saved model "${modelName}" v${version} to S3 (resource=${resourceName}/plugin=ml/models/${modelName}/v${version})`);
            }
            else {
                // Save without versioning (legacy behavior)
                await storage.set(storage.getPluginKey(resourceName, 'models', modelName, 'latest'), {
                    modelName,
                    type: 'model',
                ...
                this.logger.debug({ modelName, resourceName }, `Saved model "${modelName}" to S3 (resource=${resourceName}/plugin=ml/models/${modelName}/latest)`);
            }
            ...
            const compatibilityData = enableVersioning
                ...
                : exportedModel;
            await storage.set(`model_${modelName}`, {
                modelName,
                type: 'model',
    ...
    async _saveTrainingData(modelName, rawData) {
        try {
            ...
            if (enableVersioning) {
                ...
                const [ok, err, existing] = await tryFn(() => storage.get(storage.getPluginKey(resourceName, 'training', 'history', modelName)));
                let history = [];
                ...
                if (newSamples.length > 0) {
                    await storage.set(storage.getPluginKey(resourceName, 'training', 'data', modelName, `v${version}`), {
                        modelName,
                        version,
                ...
                await storage.set(storage.getPluginKey(resourceName, 'training', 'history', modelName), {
                    modelName,
                ...
                this.logger.debug({ modelName, version, newSamples: newSamples.length, totalSamples: processedData.length, resourceName }, `Saved training data for "${modelName}" v${version}: ${newSamples.length} new samples (total: ${processedData.length}, storage: resource=${resourceName}/plugin=ml/training/data/${modelName}/v${version})`);
            }
            else {
                // Legacy: Replace training data (non-incremental)
                await storage.set(storage.getPluginKey(resourceName, 'training', 'data', modelName, 'latest'), {
                    modelName,
                    type: 'training_data',
                ...
                this.logger.debug({ modelName, sampleCount: processedData.length, resourceName }, `Saved training data for "${modelName}" (${processedData.length} samples) to S3 (resource=${resourceName}/plugin=ml/training/data/${modelName}/latest)`);
            }
        }
        catch (error) {
    ...
    async _loadModel(modelName) {
        try {
            ...
            if (enableVersioning) {
                ...
                const [okRef, errRef, activeRef] = await tryFn(() => storage.get(storage.getPluginKey(resourceName, 'metadata', modelName, 'active')));
                if (okRef && activeRef && activeRef.version) {
                    ...
                    if (ok && versionData && versionData.modelData) {
                        ...
                        this.logger.debug({ modelName, version, resourceName }, `Loaded model "${modelName}" v${version} (active) from S3 (resource=${resourceName}/plugin=ml/models/${modelName}/v${version})`);
                        return;
                    }
                }
                ...
                if (versionInfo && versionInfo.latestVersion > 0) {
                    ...
                    if (ok && versionData && versionData.modelData) {
                        await this.models[modelName].import(versionData.modelData);
                        this.logger.debug({ modelName, version }, `Loaded model "${modelName}" v${version} (latest) from S3`);
                        return;
                    }
                }
                ...
                const [ok, err, record] = await tryFn(() => storage.get(storage.getPluginKey(resourceName, 'models', modelName, 'latest')));
                ...
                this.logger.debug({ modelName, resourceName }, `Loaded model "${modelName}" from S3 (resource=${resourceName}/plugin=ml/models/${modelName}/latest)`);
            }
        }
        catch (error) {
    ...
    async getTrainingData(modelName, version = null) {
        try {
            ...
            if (!enableVersioning) {
                ...
                const [ok, err, record] = await tryFn(() => storage.get(storage.getPluginKey(resourceName, 'training', 'data', modelName, 'latest')));
                if (!ok || !record) {
            ...
            const [okHistory, errHistory, historyData] = await tryFn(() => storage.get(storage.getPluginKey(resourceName, 'training', 'history', modelName)));
            ...
            for (const entry of historyEntries) {
                ...
                if (entry.storageKey && entry.newSamples > 0) {
                    const [ok, err, versionData] = await tryFn(() => storage.get(storage.getPluginKey(resourceName, 'training', 'data', modelName, `v${entry.version}`)));
    ...
    async _deleteModel(modelName) {
        try {
            ...
            this.logger.debug({ modelName, resourceName }, `Deleted model "${modelName}" from S3 (resource=${resourceName}/plugin=ml/models/${modelName}/)`);
        }
        catch (error) {
    ...
    async _deleteTrainingData(modelName) {
        try {
            ...
            this.logger.debug({ modelName, resourceName }, `Deleted training data for "${modelName}" from S3 (resource=${resourceName}/plugin=ml/training/)`);
        }
        catch (error) {
    ...
    async rollbackVersion(modelName, version = null) {
        ...
        this.logger.debug({ modelName, previousVersion: versionInfo.currentVersion, targetVersion }, `Rolled back model "${modelName}" from v${versionInfo.currentVersion} to v${targetVersion}`);
        return {
            modelName,
