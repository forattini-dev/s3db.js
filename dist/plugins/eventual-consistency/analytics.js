...
export async function updateAnalytics(transactions, analyticsResource, config) {
    if (!analyticsResource || transactions.length === 0)
        return;
    if (!config.field) {
    ...
    try {
        ...
        await Promise.all(Object.entries(byHour).map(([cohort, txns]) => upsertAnalytics('hour', cohort, txns, analyticsResource, config)));
        if (config.analyticsConfig.rollupStrategy === 'incremental') {
            const uniqueHours = Object.keys(byHour);
            await Promise.all(uniqueHours.map(cohortHour => rollupAnalytics(cohortHour, analyticsResource, config)));
        }
    }
    catch (error) {
...
async function upsertAnalytics(period, cohort, transactions, analyticsResource, config) {
    ...
    const [existingOk, existingErr, existing] = await tryFn(() => analyticsResource.get(id));
    if (existingOk && existing) {
        ...
        await tryFn(() => analyticsResource.update(id, {
        ...
        await tryFn(() => analyticsResource.insert({
            id,
            field: config.field,
            period,
            cohort,
...
async function rollupPeriod(period, cohort, sourcePrefix, analyticsResource, config) {
    ...
    const [ok, err, allAnalytics] = await tryFn(() => analyticsResource.list());
    if (!ok || !allAnalytics)
        return;
    let sourceAnalytics;
    if (period === 'week') {
        sourceAnalytics = allAnalytics.filter(a => {
            if (a.period !== sourcePeriod)
                return false;
        ...
        sourceAnalytics = allAnalytics.filter(a => a.period === sourcePeriod && a.cohort.startsWith(sourcePrefix));
    }
...
export async function getAnalytics(resourceName, field, options, fieldHandlers) {
    ...
    const [ok, err, allAnalytics] = await tryFn(() => handler.analyticsResource.list());
    if (!ok || !allAnalytics) {
        return [];
    }
...
async function getAnalyticsForRecord(resourceName, field, recordId, options, handler) {
    ...
    const [okTrue, errTrue, appliedTransactions] = await tryFn(() => handler.transactionResource.query({
        originalId: recordId,
        applied: true
    }));
    const [okFalse, errFalse, pendingTransactions] = await tryFn(() => handler.transactionResource.query({
        originalId: recordId,
        applied: false
    }));
    let allTransactions = [
...
export async function getTopRecords(resourceName, field, options, fieldHandlers) {
    ...
    const [ok, err, transactions] = await tryFn(() => handler.transactionResource.list());
    if (!ok || !transactions) {
        return [];
    }
...
export async function getRawEvents(resourceName, field, options, fieldHandlers) {
    ...
    const { recordId, startDate, endDate, cohortDate, cohortHour, cohortMonth, applied, operation, limit } = options;
    const query = {};
    ...
    const [ok, err, allTransactions] = await tryFn(() => handler.transactionResource.query(query));
    ...
    if (startDate && endDate) {
        ...
        filtered = filtered.filter(t => t[cohortField] && t[cohortField] >= startDate && t[cohortField] <= endDate);
    }
    else if (startDate) {
        const isHourly = startDate.length > 10;
        const cohortField = isHourly ? 'cohortHour' : 'cohortDate';
