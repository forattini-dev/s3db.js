...
export class SingleEliminationFormat extends BaseFormat {
    ...
    isComplete(bracket, matches) {
        ...
        const finalComplete = matches.some(m => (m.metadata?.matchRef || m.id) === finalMatch.id && m.status === 'completed');
        if (bracket.thirdPlaceMatch) {
            const thirdPlaceComplete = matches.some(m => (m.metadata?.matchRef || m.id) === '3RD' && m.status === 'completed');
    ...
    getWinner(bracket, matches) {
        if (!this.isComplete(bracket, matches))
            return null;
        const finalRound = bracket.matches[bracket.matches.length - 1];
        const finalMatch = matches.find(m => (m.metadata?.matchRef || m.id) === finalRound[0].id);
    ...
    getCurrentPhase(bracket, matches) {
        ...
        if (completedRounds >= bracket.rounds - 1)
            return 'finals';
        if (completedRounds >= bracket.rounds - 2)
            return 'semifinals';
        if (completedRounds >= bracket.rounds - 3)
