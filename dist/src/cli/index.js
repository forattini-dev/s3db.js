...
program
    ...
    try {
        ...
        if (!options.quiet) { // Assuming a quiet option might exist
...
async function consoleREPL(options) {
    ...
    let currentResource = null; // Placeholder for Resource type
...
`);
            ...
            for (const [field, type] of Object.entries(schema.attributes)) {
                const typeStr = type.toString(); // Cast to any to get toString()
                let bqType = 'STRING';
                if (typeStr.includes('number'))
                    ...
                    bqType = 'ARRAY<STRING>'; // Simplified
                else if (typeStr.includes('object'))
                    bqType = 'JSON';
                else if (typeStr.includes('embedding'))
                    bqType = 'ARRAY<FLOAT64>';
    ...
    try {
        ...
        if (schema.partitions && Object.keys(schema.partitions).length > 0) {
            ...
            for (const [name, config] of Object.entries(schema.partitions)) {
                if (options.partition && name !== options.partition)
                    continue;
    ...
    try {
        ...
        if (options.file) {
            ...
            for (const r of resources) {
                ...
                if (factory) {
                    specs[r.name] = options.count ? parseInt(options.count) : 10;
                }
            }
...
migrate
    ...
    try {
        ...
        const manager = new MigrationManager(db, options.dir);
        await manager.init();
        const step = options.step ? parseInt(options.step) : 1;
