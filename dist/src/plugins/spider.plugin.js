...
export class SpiderPlugin extends Plugin {
    ...
    async initialize() {
        ...
        try {
            ...
            if (this.config.ttl.enabled) {
                const targetsResourceName = this.resourceNames.targets;
            ...
            throw new PluginError(`SpiderPlugin initialization failed: ${error.message}`, { cause: error });
        }
    }
    /**
                ...
                if (this._shouldExecuteCategory(task, 'assets')) {
                    try {
                        ...
                        if (seoAnalysis && seoAnalysis.assets) {
                            ...
                            assetsAnalysis = this.seoAnalyzer._extractAssets(new DOMParser().parseFromString(html, 'text/html'), task.url);
                        }
                    ...
                    if (currentDepth < this.config.discovery.maxDepth && !this.linkDiscoverer.isLimitReached()) {
                        try {
                            ...
                            if (discoveredLinks.length > 0) {
                                this.logger.debug({ url: task.url, count: discoveredLinks.length }, `Discovered ${discoveredLinks.length} links from ${task.url}`);
    ...
    async getQueueStatus() {
        if (!this.queuePlugin)
            return null;
        return await this.queuePlugin.getStatus?.() ?? this.queuePlugin.getStats?.();
    }
    /**
    ...
    enablePersistence(config = {}) {
        ...
        if (config.saveResults !== undefined)
            this.config.persistence.saveResults = config.saveResults;
        if (config.saveSEOAnalysis !== undefined)
            this.config.persistence.saveSEOAnalysis = config.saveSEOAnalysis;
        if (config.saveTechFingerprint !== undefined)
            this.config.persistence.saveTechFingerprint = config.saveTechFingerprint;
        if (config.saveSecurityAnalysis !== undefined)
            this.config.persistence.saveSecurityAnalysis = config.saveSecurityAnalysis;
        if (config.saveScreenshots !== undefined)
            this.config.persistence.saveScreenshots = config.saveScreenshots;
        if (config.savePerformanceMetrics !== undefined)
            this.config.persistence.savePerformanceMetrics = config.savePerformanceMetrics;
...
//# sourceMappingURL=spider.plugin.js.map