...
import { createCloudDriver, validateCloudDefinition, listCloudDrivers, registerCloudDriver, BaseCloudDriver } from './cloud-inventory/index.js';
...
export class CloudInventoryPlugin extends Plugin {
    ...
    constructor(options = {}) {
        ...
        const normalizedClouds = normalizeCloudDefinitions(Array.isArray(opts.clouds) ? opts.clouds : [], (level, message, meta) => pendingLogs.push({ level, message, meta: meta ?? {} }));
        const { resourceNames = {}, discovery = {}, logger, scheduled, lock = {}, terraform = {}, ...rest } = opts;
    ...
    async exportToTerraformState(options = {}) {
        ...
        const { resourceTypes = [], providers = [], cloudId = null, ...exportOptions } = options;
    ...
    async _initializeDrivers() {
        for (const cloudDef of this.config.clouds) {
            ...
            if (this.cloudDrivers.has(driverId))
                continue;
    ...
    _scheduleJob(schedule, handler) {
        if (!this._cron)
            return;
        const job = this._cron.schedule(schedule.cron, handler, { timezone: schedule.timezone });
        if (job?.start) {
            job.start();
        }
    ...
    _normalizeResource(cloudDefinition, entry) {
        ...
        const configuration = ensureObject(entry.configuration ??
            entry.state ??
            entry.attributes ??
            entry);
        const normalized = {
...
function computeDiff(previousConfig = {}, nextConfig = {}) {
    ...
    if (!Object.keys(diff.added).length)
        delete diff.added;
    if (!Object.keys(diff.removed).length)
        delete diff.removed;
...
//# sourceMappingURL=cloud-inventory.plugin.js.map