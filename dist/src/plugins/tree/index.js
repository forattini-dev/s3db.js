...
export class TreePlugin extends Plugin {
    ...
    constructor(options = {}) {
        ...
        const { resources = [], driver = 'nested-set', leftField = 'lft', rightField = 'rgt', depthField = 'depth', parentField = 'parentId', treeField = null, rootParentValue = null, autoRebuild = false } = opts;
        this.config = {
    ...
    _installResourceMethods() {
        for (const resource of Object.values(this.database.resources)) {
            if (!this.config.resources.includes(resource.name))
                continue;
            if (this._resourceTreeNamespaces.has(resource.name))
                continue;
    ...
    _enrichNodeWithTree(node, resourceName) {
        ...
        const nodeTreeNamespace = {
            parent: () => driver.getNode(resourceName, node[plugin.config.parentField]).catch(() => null),
