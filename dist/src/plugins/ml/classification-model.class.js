...
export class ClassificationModel extends BaseModel {
    ...
    buildModel() {
        ...
        if (numClasses < 2) {
            throw new ModelConfigError('Classification requires at least 2 classes', { model: this.config.name, numClasses });
        }
        if (!this.tf) {
    ...
    _prepareData(data) {
        ...
        for (const record of data) {
            ...
            if (!(targetClass in this.classToIndex)) {
                throw new DataValidationError(`Unknown class "${targetClass}" in training data`, { model: this.config.name, targetClass, knownClasses: this.classes });
            }
    ...
    _prepareTargetTensor(targets) {
        ...
        if (isBinary) {
            ...
            return this.tf.oneHot(this.tf.tensor1d(targets, 'int32'), this.classes.length);
        }
    }
    /**
    ...
    async predict(input) {
        ...
        try {
            ...
            return {
                ...
                } : Object.fromEntries(this.classes.map((cls, idx) => [cls, predictionArray[idx]]))
            };
        }
        catch (error) {
            this.stats.errors++;
...
//# sourceMappingURL=classification-model.class.js.map