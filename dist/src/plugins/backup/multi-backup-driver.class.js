...
export default class MultiBackupDriver extends BaseBackupDriver {
    ...
    async upload(filePath, backupId, manifest) {
        ...
        if (strategy === 'priority') {
            for (const { driver, config, index } of this.drivers) {
                const [ok, err, result] = await tryFn(() => driver.upload(filePath, backupId, manifest));
                if (ok) {
    ...
    async download(backupId, targetPath, metadata) {
        ...
        for (const destMetadata of destinations) {
            if (destMetadata.status !== 'success')
                continue;
            const driverInstance = this.drivers.find(d => d.index === destMetadata.destination);
            if (!driverInstance)
                continue;
            const [ok, err, result] = await tryFn(() => driverInstance.driver.download(backupId, targetPath, destMetadata));
            if (ok) {
    ...
    async delete(backupId, metadata) {
        ...
        for (const destMetadata of destinations) {
            ...
            if (!driverInstance)
                continue;
            const [ok, err] = await tryFn(() => driverInstance.driver.delete(backupId, destMetadata));
            if (ok) {
                successCount++;
    ...
    async list(options = {}) {
        const allLists = await Promise.allSettled(this.drivers.map(({ driver, index }) => driver.list(options).catch(err => {
    ...
    async verify(backupId, expectedChecksum, metadata) {
        ...
        for (const destMetadata of destinations) {
            ...
            if (!driverInstance)
                continue;
            const [ok, , isValid] = await tryFn(() => driverInstance.driver.verify(backupId, expectedChecksum, destMetadata));
            if (ok && isValid) {
    ...
    async cleanup() {
        await Promise.all(this.drivers.map(({ driver }) => tryFn(() => driver.cleanup()).catch(() => { })));
    }
    getStorageInfo() {
        return {
...
//# sourceMappingURL=multi-backup-driver.class.js.map