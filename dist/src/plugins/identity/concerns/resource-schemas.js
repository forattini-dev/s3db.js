...
export function validateExtraAttributes(baseAttributes, userAttributes, resourceType) {
    ...
    for (const fieldName of Object.keys(userAttributes)) {
        if (baseAttributes[fieldName]) {
            errors.push(`Cannot override base attribute '${fieldName}' in ${resourceType} resource. ` +
    ...
    for (const [fieldName, fieldSchema] of Object.entries(userAttributes)) {
        ...
        if (isOptional && !hasDefault) {
            errors.push(`Extra attribute '${fieldName}' in ${resourceType} resource is optional but has no default value. ` +
...
export function mergeResourceConfig(baseConfig, userConfig = {}, resourceType) {
    if (userConfig.attributes) {
        const validation = validateExtraAttributes(baseConfig.attributes || {}, userConfig.attributes, resourceType);
        if (!validation.valid) {
            const errorMsg = [
...
export function validateResourcesConfig(resourcesConfig) {
    ...
    if (!resourcesConfig.users) {
        errors.push('IdentityPlugin requires "resources.users" configuration.\n' +
    ...
    if (!resourcesConfig.tenants) {
        errors.push('IdentityPlugin requires "resources.tenants" configuration.\n' +
    ...
    if (!resourcesConfig.clients) {
        errors.push('IdentityPlugin requires "resources.clients" configuration.\n' +
...
//# sourceMappingURL=resource-schemas.js.map