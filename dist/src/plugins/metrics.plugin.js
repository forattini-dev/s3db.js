...
export class MetricsPlugin extends Plugin {
    ...
    metrics;
    resourceNames;
    metricsResource = null;
    errorsResource = null;
    performanceResource = null;
    flushJobName = null;
    flushTimer = null;
    metricsServer = null;
    ...
    async onInstall() {
        ...
        const [ok] = await tryFn(async () => {
            ...
            this.metricsResource = ok1
                ? metricsResource
                : (this.database.resources[this.resourceNames.metrics] ?? null);
            ...
            this.errorsResource = ok2
                ? errorsResource
                : (this.database.resources[this.resourceNames.errors] ?? null);
            ...
            this.performanceResource = ok3
                ? performanceResource
                : (this.database.resources[this.resourceNames.performance] ?? null);
        });
        if (!ok) {
            this.metricsResource = this.database.resources[this.resourceNames.metrics] ?? null;
            this.errorsResource = this.database.resources[this.resourceNames.errors] ?? null;
            this.performanceResource = this.database.resources[this.resourceNames.performance] ?? null;
        }
    ...
    _setupOperationPoolListeners() {
        ...
        client.on('pool:taskCompleted', (_task, _result) => {
    ...
    async stop() {
        ...
        if (this.flushTimer) {
            const clearFn = (globalThis?.originalClearInterval ||
    ...
    installMetricsHooks() {
        ...
        for (const resource of Object.values(db.resources || {})) {
            if (this.isInternalResource(resource.name)) {
                continue;
            }
            this.installResourceHooks(resource);
        ...
        const self = this;
        db.createResource = async function (...args) {
    ...
    installResourceHooks(resource) {
        ...
        resource.insert = async function (...args) {
            ...
            if (!ok)
                self.recordError(resource.name, 'insert', err);
            if (!ok)
                throw err;
            return result;
        };
        resource.update = async function (...args) {
            ...
            if (!ok)
                self.recordError(resource.name, 'update', err);
            if (!ok)
                throw err;
            return result;
        };
        resource.delete = async function (...args) {
            ...
            if (!ok)
                self.recordError(resource.name, 'delete', err);
            if (!ok)
                throw err;
            return result;
        };
        ...
        resource.get = async function (...args) {
            ...
            if (!ok)
                self.recordError(resource.name, 'get', err);
            if (!ok)
                throw err;
            return result;
        };
        ...
        resource.getAll = async function () {
            ...
            if (!ok)
                self.recordError(resource.name, 'list', err);
            if (!ok)
                throw err;
            return result;
        };
        ...
        resource.count = async function (...args) {
            ...
            if (!ok)
                self.recordError(resource.name, 'count', err);
            if (!ok)
                throw err;
            return result;
        };
    ...
    startFlushTimer() {
        ...
        if (this.config.flushInterval > 0) {
            ...
            cronManager.scheduleInterval(this.config.flushInterval, () => this.flushMetrics().catch(() => { }), jobName).then((task) => {
    ...
    async getMetrics(options = {}) {
        const { type = 'operation', resourceName, operation, startDate, endDate, limit = 100, offset = 0 } = options;
        if (!this.metricsResource)
            return [];
        ...
        let filtered = allMetrics.filter((metric) => {
            if (type && metric.type !== type)
                return false;
            if (resourceName && metric.resourceName !== resourceName)
                return false;
            if (operation && metric.operation !== operation)
                return false;
            if (startDate && new Date(metric.timestamp) < new Date(startDate))
                return false;
            if (endDate && new Date(metric.timestamp) > new Date(endDate))
                return false;
            return true;
        });
    ...
    async getErrorLogs(options = {}) {
        if (!this.errorsResource)
            return [];
        const { resourceName, operation, startDate, endDate, limit = 100, offset = 0 } = options;
        const allErrors = await this.errorsResource.getAll();
        let filtered = allErrors.filter((error) => {
            if (resourceName && error.resourceName !== resourceName)
                return false;
            if (operation && error.operation !== operation)
                return false;
            if (startDate && new Date(error.timestamp) < new Date(startDate))
                return false;
            if (endDate && new Date(error.timestamp) > new Date(endDate))
                return false;
            return true;
        });
        filtered.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    ...
    async getPerformanceLogs(options = {}) {
        ...
        let filtered = allPerformance.filter((perf) => {
            if (resourceName && perf.resourceName !== resourceName)
                return false;
            if (operation && perf.operation !== operation)
                return false;
            if (startDate && new Date(perf.timestamp) < new Date(startDate))
                return false;
            if (endDate && new Date(perf.timestamp) > new Date(endDate))
                return false;
            return true;
        });
        filtered.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    ...
    async cleanupOldData() {
        ...
        if (this.metricsResource) {
            for (const dateStr of datesToDelete) {
                const [ok, , oldMetrics] = await tryFn(() => this.metricsResource.query({ createdAt: dateStr }));
                if (ok && oldMetrics) {
        ...
        if (this.errorsResource) {
            for (const dateStr of datesToDelete) {
                const [ok, , oldErrors] = await tryFn(() => this.errorsResource.query({ createdAt: dateStr }));
                if (ok && oldErrors) {
        ...
        if (this.performanceResource) {
            for (const dateStr of datesToDelete) {
                const [ok, , oldPerformance] = await tryFn(() => this.performanceResource.query({ createdAt: dateStr }));
    ...
    async _setupPrometheusExporter() {
        ...
        const registry = this.database.pluginRegistry;
        ...
        if (mode === 'auto') {
            ...
            this.logger.warn({ mode }, `[Metrics Plugin] Unknown prometheus.mode="${mode}". Valid modes: auto, integrated, standalone`);
        }
    }
    async _setupIntegratedMetrics(apiPlugin) {
        const port = apiPlugin.config?.port || 3000;
        const path = this.config.prometheus.path;
        this.logger.debug({ port, path, mode: 'integrated', auth: 'public' }, `Prometheus metrics will be available at http://localhost:${port}${path} (integrated mode)`);
    ...
    async _setupStandaloneMetrics() {
            ...
            if (req.url === path && req.method === 'GET') {
                if (enforceIpAllowlist && isIpAllowed) {
                    ...
                    if (!clientIp || !isIpAllowed(clientIp, ipAllowlist)) {
                        this.logger.warn({ clientIp: clientIp || 'unknown', endpoint: '/metrics' }, `Blocked /metrics request from unauthorized IP: ${clientIp || 'unknown'}`);
        ...
        this.metricsServer.listen(port, '0.0.0.0', () => {
            ...
            this.logger.debug({ port, path, mode: 'standalone', ipFilterEnabled: enforceIpAllowlist, ipRangesCount: ipAllowlist.length }, `Prometheus metrics available at http://0.0.0.0:${port}${path} (standalone mode)${ipFilter}`);
        });
