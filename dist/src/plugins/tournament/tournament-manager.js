...
export class TournamentManager {
    ...
    async create(options) {
        const { name, organizerId, format, participantType = 'team', participantResource = null, config = {}, metadata = {} } = options;
        if (!name)
        ...
        const FormatClass = formatInstance.constructor;
    ...
    async updateBracket(tournamentId, completedMatch) {
        ...
        if (!tournament)
            throw new Error('Tournament not found');
        const formatInstance = createFormat(tournament.format, tournament.config);
        const { bracket: updatedBracket, newMatches } = formatInstance.onMatchComplete(tournament.bracket, completedMatch);
