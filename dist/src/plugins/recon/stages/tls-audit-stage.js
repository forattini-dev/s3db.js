...
export class TlsAuditStage {
    ...
    async execute(target, featureConfig = {}) {
        ...
        const result = await this.commandRunner.runRedBlue('tls', 'intel', 'audit', hostPort, {
    ...
    _cipherStrength(cipher) {
        ...
        if (c.includes('256') || c.includes('chacha20'))
            return 'strong';
        if (c.includes('128'))
            return 'medium';
        if (c.includes('rc4') || c.includes('des') || c.includes('null'))
            return 'weak';
        return 'unknown';
    }
    _parseRawAudit(raw) {
...
//# sourceMappingURL=tls-audit-stage.js.map