...
export class WhoisStage {
    ...
    async execute(target, featureConfig = {}) {
        ...
        const result = await this.commandRunner.runRedBlue('recon', 'domain', 'whois', domain, {
    ...
    _parseRawWhois(raw) {
        ...
        for (const [key, pattern] of Object.entries(patterns)) {
            ...
            if (match) {
                if (key === 'domain')
                    result.domain = match[1].trim();
                else if (key === 'registrar')
                    result.registrar = match[1].trim();
                else if (key === 'createdDate')
                    result.dates.created = this._parseDate(match[1].trim());
                else if (key === 'updatedDate')
                    result.dates.updated = this._parseDate(match[1].trim());
                else if (key === 'expiryDate')
                    result.dates.expiration = this._parseDate(match[1].trim());
                else if (key === 'dnssec')
                    result.dnssec = match[1].trim().toLowerCase();
            }
        }
...
//# sourceMappingURL=whois-stage.js.map