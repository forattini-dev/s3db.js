...
export class DiffDetector {
    static detect(previousReport, currentReport) {
        ...
        diff.changes.certificate = this._detectCertificateChanges(previousReport.results?.certificate, currentReport.results?.certificate);
        diff.changes.ports = this._detectPortChanges(previousReport.results?.ports, currentReport.results?.ports);
        diff.changes.subdomains = this._detectSubdomainChanges(previousReport.results?.subdomains, currentReport.results?.subdomains);
        diff.changes.paths = this._detectPathChanges(previousReport.results?.webDiscovery, currentReport.results?.webDiscovery);
        diff.changes.technologies = this._detectTechnologyChanges(previousReport.results?.fingerprint, currentReport.results?.fingerprint);
        diff.changes.security = this._detectSecurityChanges(previousReport.results?.tlsAudit, currentReport.results?.tlsAudit, previousReport.results?.vulnerability, currentReport.results?.vulnerability);
    ...
    static _detectDnsChanges(oldDns, newDns) {
        ...
        const extractMx = (mx) => typeof mx === 'object' ? mx.exchange : String(mx);
