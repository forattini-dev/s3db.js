...
export class NeuralNetworkModel extends BaseModel {
   ...
   * @private
   */
  private _validateLayersConfig(): void {
   ...
   * @private
   */
  private _isValidActivation(activation: string): boolean {
   ...
   */
  setOutput(outputConfig: { activation?: string; units?: number; loss?: string; metrics?: string[] }): void {
    if (this.model) {
      throw new ModelConfigError(
  ...
  getArchitecture(): any {
    return {
      ...
      hiddenLayers: this.config.modelConfig.layers.map((layer: LayerConfig, index: number) => ({
        index,
        units: layer.units,
   ...
   */
  async trainWithEarlyStopping(data: any[], earlyStoppingConfig: any = {}): Promise<any> {
    const {
      patience = 10,
      minDelta = 0.001,
      monitor = 'val_loss',
