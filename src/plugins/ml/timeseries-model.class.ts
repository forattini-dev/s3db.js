...
export class TimeSeriesModel extends BaseModel {
  ...
  override _prepareData(data: any[]): { xs: any; ys: any } {
    ...
    // Convert to tensors
    return {
      xs: this.tf!.tensor3d(sequences as any), // [samples, lookback, features]
   ...
   */
  override async predict(sequence: any[]): Promise<any> {
    if (!this.isTrained) {
    ...
    try {
      ...
      const inputTensor = this.tf.tensor3d([normalizedSequence] as any);
   ...
   */
  async predictMultiStep(initialSequence: any[], steps: number = 1): Promise<number[]> {
    if (!this.isTrained) {
      throw new ModelNotTrainedError(`Model "${this.config.name}" is not trained yet`, {
        model: this.config.name
      });
   ...
   */
  async calculateMAPE(data: any[]): Promise<number> {
    if (!this.isTrained) {
      throw new ModelNotTrainedError(`Model "${this.config.name}" is not trained yet`, {
        model: this.config.name
      });
