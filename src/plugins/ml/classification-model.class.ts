...
import { ModelConfigError, DataValidationError, ModelNotTrainedError, PredictionError } from '../ml.errors.js'; // Import from TS version
...
export class ClassificationModel extends BaseModel {
   ...
   * @private
   */
  override _prepareData(data: any[]): { xs: any; ys: any } {
    ...
    this.classes.forEach((cls: any, idx: number) => {
    ...
    this._calculateNormalizer(features, targets); // Targets here are indices, not values for normalization
   ...
   * @protected
   */
  override _prepareTargetTensor(targets: number[]): any {
    ...
    if (isBinary) {
      ...
      return this.tf.oneHot(this.tf.tensor1d(targets as any, 'int32'), this.classes.length);
    }
  }
   ...
   * @private
   */
  override _calculateNormalizer(features: number[][], targets: number[]): void {
   ...
   */
  override async predict(input: any): Promise<any> {
    if (!this.isTrained) {
    ...
    try {
      ...
      return {
        ...
        probabilities: isBinary ? {
          ...
          this.classes.map((cls: any, idx: number) => [cls, predictionArray[idx]])
        )
      };
    } catch (error: any) {
      this.stats.errors++;
   ...
   */
  async calculateConfusionMatrix(data: any[]): Promise<any> {
    if (!this.isTrained) {
      throw new ModelNotTrainedError(`Model "${this.config.name}" is not trained yet`, {
        model: this.config.name
      });
    ...
    for (const record of data) {
      ...
      if (matrix[actual] && matrix[actual][prediction] !== undefined) {
        matrix[actual][prediction]++;
      } else {
        // Handle cases where prediction or actual might be unexpected
        this.logger.warn(`Unexpected class encountered: actual=${actual}, prediction=${prediction}`);
      }
    }
    ...
    for (const cls of this.classes) {
      totalCorrect += matrix[cls]?.[cls] || 0;
      total += Object.values(matrix[cls] || {}).reduce((sum, val) => sum + (val as number), 0);
    }
   ...
   */
  override async export(): Promise<any> {
   ...
   */
  override async import(data: any): Promise<void> {
