...
export class EmailService {
  ...
  private _baseTemplate({ title, preheader, content }: BaseTemplateOptions): string {
...
<body>
  ...
  async sendPasswordResetEmail({ to, name, resetToken, expiresIn = 60 }: PasswordResetEmailOptions): Promise<SendEmailResult> {
  ...
  async sendEmailVerificationEmail({ to, name, verificationToken, expiresIn = 24 }: EmailVerificationOptions): Promise<SendEmailResult> {
    const { baseUrl } = this.config.templates;
  ...
  async sendWelcomeEmail({ to, name }: WelcomeEmailOptions): Promise<SendEmailResult> {
    const { baseUrl } = this.config.templates;
