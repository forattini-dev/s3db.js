...
import { AdminDashboardPage, type DashboardStats, type RecentUser } from './pages/admin/dashboard.js';
import { AdminClientsPage } from './pages/admin/clients.js';
import { AdminClientFormPage } from './pages/admin/client-form.js';
import { AdminUsersPage, type AdminUser } from './pages/admin/users.js';
import { AdminUserFormPage, type EditUser } from './pages/admin/user-form.js';
...
import { createRedirectRateLimitMiddleware, RateLimiter as RateLimiterClass } from '../concerns/rate-limit.js';
import type { ThemeConfig, BaseLayoutUser } from './layouts/base.js';
import { SessionManager as SessionManagerClass } from '../session-manager.js';
...
export interface SessionManager {
  ...
  validateSession(sessionId: string): Promise<{ valid: boolean; session?: unknown }>;
  ...
  getUserSessions(userId: string): Promise<Array<{ id: string; [key: string]: unknown }>>;
}

export interface KeyManager {
  createToken(payload: Record<string, unknown>, expiresIn: string): string;
  verifyToken(token: string): Promise<{ payload: Record<string, unknown> } | null>;
}

export interface MFAManager {
  generateEnrollment(email: string): { secret: string; qrCodeUrl: string; backupCodes: string[] };
...
export interface EmailService {
  ...
  sendPasswordResetEmail(options: { to: string; name: string; resetToken: string; expiresIn: number } | string, data?: { name: string; resetUrl: string }): Promise<void>;
  sendEmailVerificationEmail(options: { to: string; name: string; verificationToken: string }): Promise<void>;
}
...
export interface Resource<T = Record<string, unknown>> {
  ...
  list(options?: { limit?: number }): Promise<T[]>;
...
export interface IdentityPlugin {
  ...
  oauth2Server?: { keyManager?: KeyManager };
  ...
  authenticateWithPassword(options: { email: string; password: string; user: UserRecord }): Promise<{ success: boolean; statusCode?: number; error?: string; user?: UserRecord }>;
...
export function registerUIRoutes(app: Hono, plugin: IdentityPlugin): void {
  ...
  const verifyMfaChallengeToken = async (token: string): Promise<Record<string, unknown> | null> => {
    if (!keyManager || !token) {
      return null;
    }
  ...
  const logAudit = async (event: string, data: Record<string, unknown>): Promise<void> => {
  ...
  app.get('/login', async (c) => {
    ...
    const PageComponent = getPageComponent(customPages, 'login', LoginPage as PageComponentType);
  ...
  const buildRateLimitRedirect = (retryAfter: number): string => {
  ...
  // ============================================================================
  const loginHandler = async (c: Context): Promise<Response> => {
    try {
      ...
      let challengePayload: Record<string, unknown> | null = null;
      ...
      if (usingChallenge) {
        ...
        normalizedEmail = ((challengePayload.email as string) || '').toLowerCase();
      ...
      if (usingChallenge) {
        const [okUser, errUser, challengeUser] = await tryFn(() =>
          usersResource.get(challengePayload!.userId as string)
        );
        ...
        const [okQuery, , users] = await tryFn(() =>
          usersResource.query({ email: normalizedEmail } as Partial<UserRecord>)
        );
      ...
      if (config.mfa.enabled && plugin.mfaDevicesResource) {
        const [okMFA, errMFA, devices] = await tryFn(() =>
          plugin.mfaDevicesResource!.query({ userId: user!.id, verified: true } as Partial<MFADevice>)
        );
  ...
  if (loginLimiter) {
    const loginRateMiddleware = createRedirectRateLimitMiddleware(
      loginLimiter as unknown as RateLimiterClass,
      getClientIp,
  ...
  app.get('/login/mfa', async (c) => {
    ...
    try {
      ...
      const [okUser, , challengeUser] = await tryFn(() =>
        usersResource.get(payload.userId as string)
      );
  ...
  app.get('/register', async (c) => {
    ...
    const PageComponent = getPageComponent(customPages, 'register', RegisterPage as PageComponentType);
    return c.html(PageComponent({
      error: error ? decodeURIComponent(error) : null,
      email,
      name,
  ...
  app.post('/register', async (c) => {
    ...
    try {
      const body = await c.req.parseBody();
      const { name, email, password, confirm_password, agree_terms } = body as Record<string, string>;
      ...
      const [okCheck, , existingUsers] = await tryFn(() =>
        usersResource.query({ email: normalizedEmail } as Partial<UserRecord>)
      );
  ...
  app.get('/forgot-password', async (c) => {
    ...
    const PageComponent = getPageComponent(customPages, 'forgotPassword', ForgotPasswordPage as PageComponentType);
    return c.html(PageComponent({
      error: error ? decodeURIComponent(error) : null,
      success: success ? decodeURIComponent(success) : null,
      email,
  ...
  app.get('/reset-password', async (c) => {
    ...
    const [okQuery, , tokens] = await tryFn(() =>
      plugin.passwordResetTokensResource.query({ token } as Partial<PasswordResetToken>)
    );
    ...
    const PageComponent = getPageComponent(customPages, 'resetPassword', ResetPasswordPage as PageComponentType);
    return c.html(PageComponent({
      error: error ? decodeURIComponent(error) : null,
      token,
      passwordPolicy: config.passwordPolicy,
  ...
  app.get('/profile', sessionAuth(sessionManagerCasted, { required: true }), async (c) => {
    try {
      ...
      const [okSessions, , allSessions] = await tryFn(() =>
      ...
      const PageComponent = getPageComponent(customPages, 'profile', ProfilePage as PageComponentType);
      return c.html(PageComponent({
        user: userData,
        sessions,
        error: error ? decodeURIComponent(error) : null,
  ...
  app.post('/profile/update', sessionAuth(sessionManagerCasted, { required: true }), async (c) => {
    try {
      ...
      const [okUser, , userData] = await tryFn(() =>
        usersResource.get(user.userId || user.id!)
      );
  ...
  app.post('/profile/logout-session', sessionAuth(sessionManagerCasted, { required: true }), async (c) => {
    try {
      ...
      const [okSession, , session] = await tryFn(() =>
      ...
      const sessionData = session as { userId: string };
  ...
  app.get('/profile/mfa/enroll', sessionAuth(sessionManagerCasted, { required: true }), async (c) => {
    ...
    try {
      ...
      const [okDevices, , devices] = await tryFn(() =>
        plugin.mfaDevicesResource!.query({ userId: userData.id, verified: true } as Partial<MFADevice>)
      );
      ...
      (c as any).set('mfaEnrollment', enrollment);
  ...
  app.post('/profile/mfa/enroll', sessionAuth(sessionManagerCasted, { required: true }), async (c) => {
    ...
    try {
      const user = (c as any).get('user') as { userId?: string; id?: string };
      const body = await c.req.parseBody();
      const { token, enrollment_secret, enrollment_backup_codes } = body as Record<string, string>;
  ...
  app.post('/profile/mfa/disable', sessionAuth(sessionManagerCasted, { required: true }), async (c) => {
    ...
    try {
      ...
      const [okDevices, , devices] = await tryFn(() =>
        plugin.mfaDevicesResource!.query({ userId: userData.id } as Partial<MFADevice>)
      );
  ...
  app.get('/admin', adminOnly(sessionManagerCasted), async (c) => {
    try {
      ...
      const stats: DashboardStats = {
        ...
        recentUsers: users.slice(-5).reverse().map(u => ({ email: u.email, name: u.name, status: u.status || 'active', createdAt: '' })) as RecentUser[],
    ...
    try {
      ...
      const [okClient, errClient, client] = await tryFn(() =>
        plugin.oauth2ClientsResource.insert({
          ...
          redirectUris: (redirectUrisArray as string[]).filter(uri => uri && uri.trim() !== ''),
          grantTypes: grantTypesArray as string[],
          allowedScopes: allowedScopesArray as string[],
          active: active === '1'
        })
      );
      ...
      if (plugin.auditPlugin) {
        await plugin.auditPlugin.log({
          ...
          metadata: {
            clientId,
            clientName: (name as string).trim(),
    ...
    try {
      ...
      if (!okUsers) {
        ...
        return c.html(AdminUsersPage({
          users: [],
          user: (c as any).get('user') as BaseLayoutUser | undefined,
      ...
      const users = (allUsers || []) as unknown as AdminUser[];
  ...
  app.get('/admin/users/:id/edit', adminOnly(sessionManagerCasted), async (c) => {
    ...
    try {
      const [okUser, , userRecord] = await tryFn(() =>
      ...
      return c.html(AdminUserFormPage({
        editUser: userRecord as unknown as EditUser,
        user: (c as any).get('user') as BaseLayoutUser | undefined,
        error: error ? decodeURIComponent(error) : null,
        config: uiConfig
      }));
  ...
  app.post('/admin/users/:id/update', adminOnly(sessionManagerCasted), async (c) => {
    ...
    try {
      const [okUser, , editUser] = await tryFn(() =>
        usersResource.get(userId)
      );
      ...
      if (email !== editUser.email) {
        const [okExists, , existingUsers] = await tryFn(() =>
          usersResource.query({ email: (email ?? '').toLowerCase().trim() } as Partial<UserRecord>)
        );

        if (okExists && existingUsers && existingUsers.length > 0) {
          return c.html(AdminUserFormPage({
            editUser: { ...editUser, name: name || editUser.name, email: email || editUser.email } as unknown as EditUser,
            user: currentUser,
      ...
      if (!okUpdate) {
        ...
        return c.html(AdminUserFormPage({
          editUser: { ...editUser, ...updates } as unknown as EditUser,
          user: currentUser,
  ...
  app.post('/admin/users/:id/disable-mfa', adminOnly(sessionManagerCasted), async (c) => {
    ...
    try {
      ...
      const [okDevices, , devices] = await tryFn(() =>
        plugin.mfaDevicesResource!.query({ userId: user.id } as Partial<MFADevice>)
      );
  ...
  app.post('/admin/users/:id/toggle-status', adminOnly(sessionManagerCasted), async (c) => {
    ...
    try {
      ...
      const [okUpdate, errUpdate] = await tryFn(() =>
        usersResource.patch(userId, { status: newStatus })
      );
  ...
  app.get('/oauth/authorize', sessionAuth(sessionManagerCasted, { required: false }), async (c) => {
    ...
    try {
      ...
      const user = (c as any).get('user') as { id: string; email?: string; name?: string } | undefined;
      if (!user) {
      ...
      const [okClient, , clients] = await tryFn(() =>
        plugin.oauth2ClientsResource.query({ clientId: client_id } as Partial<OAuth2Client>)
      );
      ...
      const PageComponent = getPageComponent(customPages, 'consent', ConsentPage as PageComponentType);
      return c.html(PageComponent({
        client,
  ...
  app.get('/verify-email', async (c) => {
    ...
    try {
      const [okUsers, , users] = await tryFn(() =>
        usersResource.query({ emailVerificationToken: token } as Partial<UserRecord>)
      );
      ...
      const verificationUpdate: Partial<UserRecord> = {
        emailVerified: true,
