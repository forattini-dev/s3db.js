...
export function ConsentPage(props: ConsentPageProps = {}): HtmlEscapedString {
  const {
    client = {} as OAuthClient,
    scopes = [],
    user = {} as BaseLayoutUser,
    responseType = '',
    redirectUri = '',
    state = '',
    codeChallenge = '',
  ...
  const scopeDetails: ScopeDetail[] = scopes
    .map(scope => ({
      scope,
  ...
  const content = html`
    <section class="mx-auto w-full max-w-5xl space-y-8 text-slate-100">
      ...
      <p class="text-center text-sm text-slate-300">
        Not ${user?.name || 'User'}? <a href="/logout" class="font-semibold text-primary transition hover:text-white">Sign out</a>
      </p>
    </section>
  `;
