...
export class PartitionAwareFilesystemCache extends FilesystemCache {
  declare config: PartitionAwareFilesystemCacheConfig;
  ...
  constructor({
    ...
    ...config
  }: PartitionAwareFilesystemCacheConfig) {
    super(config);
  ...
  protected override async _set(key: string, data: unknown, options: PartitionOptions = {}): Promise<void> {
  ...
  override async set<T>(resource: string, action: T, options?: PartitionOptions): Promise<T>;
  override async set<T>(key: string, data: T): Promise<T>;
  override async set<T>(resourceOrKey: string, actionOrData: T, options?: PartitionOptions): Promise<T> {
  ...
  override async get<T>(resource: string, action: string, options: PartitionOptions): Promise<T | undefined>;
  override async get<T>(key: string): Promise<T | undefined>;
  override async get<T>(resourceOrKey: string, action?: string, options?: PartitionOptions): Promise<T | undefined> {
    if (typeof resourceOrKey === 'string' && typeof action === 'string' && options?.partition) {
      ...
      return this._get(key, { resource: resourceOrKey, action, ...options }) as Promise<T | undefined>;
    }

    return super.get(resourceOrKey) as Promise<T | undefined>;
  }

  protected override async _get(key: string, options: PartitionOptions = {}): Promise<unknown> {
    const { resource, action, partition, partitionValues, params } = options;

    if (resource && partition) {
      const partitionKey = this._getPartitionCacheKey(resource, action, partition, partitionValues, params);
      const payload = await super._get(partitionKey) as { data?: unknown } | null;
  ...
  async clearPartition(resource: string, partition: string, partitionValues: Record<string, unknown> = {}): Promise<boolean> {
  ...
  async clearResourcePartitions(resource: string): Promise<boolean> {
  ...
  async getPartitionStats(resource: string, partition: string | null = null): Promise<PartitionStats> {
  ...
  async getCacheRecommendations(resource: string): Promise<CacheRecommendation[]> {
  ...
  async warmPartitionCache(resource: string, options: { partitions?: string[]; maxFiles?: number } = {}): Promise<number> {
  ...
  private async _trackPartitionUsage(
    resource: string,
    partition: string,
    partitionValues?: Record<string, unknown>
  ): Promise<void> {
  ...
  private async _preloadRelatedPartitions(
    _resource: string,
    _partition: string,
    _partitionValues?: Record<string, unknown>
  ): Promise<void> {
    // Implementation would go here for intelligent preloading
  }

  private _isTemporalPartition(_partition: string, partitionValues: Record<string, unknown>): boolean {
  ...
  private _sanitizePathValue(value: unknown): string {
  ...
  private _splitKeySegments(key: string): string[] {
    return key.split('/').filter(Boolean);
  }

  private async _ensurePartitionDirectoryForKey(key: string): Promise<void> {
  ...
  protected override _getFilePath(key: string): string {
  ...
  private async _calculateDirectoryStats(dir: string, stats: PartitionStats): Promise<void> {
  ...
  async loadUsageStats(): Promise<void> {
    const [ok, , content] = await tryFn(async () => {
      ...
      return JSON.parse(data) as Record<string, PartitionUsage>;
    });
  ...
  private async _saveUsageStats(): Promise<void> {
  ...
  private async _writeFileWithMetadata(filePath: string, data: unknown): Promise<boolean> {
  ...
  private async _readFileWithMetadata(filePath: string): Promise<{ data?: unknown } | null> {
    const [ok, , content] = await tryFn(async () => {
  ...
  private async _collectKeysRecursive(currentDir: string, segments: string[], result: string[]): Promise<void> {
