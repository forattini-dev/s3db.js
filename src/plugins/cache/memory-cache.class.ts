...
export class MemoryCache extends Cache {
  declare config: MemoryCacheConfig;
  logger: Logger;
  caseSensitive: boolean;
  serializer: (value: unknown) => string;
  deserializer: (str: string) => unknown;
  ...
  evictedDueToMemory: number;
  memoryPressureEvents: number;
  private _monitorHandle: ReturnType<typeof setInterval> | null;
  ...
  constructor(config: MemoryCacheConfig = {}) {
    ...
    const defaultDeserializer = (str: string): unknown => {
    ...
    this.evictionPolicy = ((config.evictionPolicy || 'lru').toLowerCase() as EvictionPolicy);
  ...
  private _recordStat(type: keyof MemoryCacheStats): void {
    if (!this.enableStats) return;
    if (Object.prototype.hasOwnProperty.call(this.stats, type)) {
      (this.stats[type] as number) += 1;
    }
  }

  private _selectEvictionCandidate(): string | null {
  ...
  private _evictKey(key: string): number {
  ...
  private _enforceMemoryLimit(incomingSize: number = 0): boolean {
  ...
  private _reduceMemoryTo(targetBytes: number): void {
  ...
  private _memoryHealthCheck(): number {
    let freedBytes = 0;
  ...
  protected override async _set(key: string, data: unknown): Promise<unknown> {
    ...
    if (this.enableCompression) {
      try {
        ...
        this.logger.warn(`[MemoryCache] Compression failed for key '${key}': ${(error as Error).message}`);
      }
    }
  ...
  protected override async _get(key: string): Promise<unknown> {
    ...
    if (rawData && typeof rawData === 'object' && (rawData as CompressedData).__compressed) {
      try {
        ...
        this.logger.warn(`[MemoryCache] Decompression failed for key '${key}': ${(error as Error).message}`);
        delete this.cache[normalizedKey];
        delete this.meta[normalizedKey];
        this._recordStat('misses');
        return null;
    ...
    try {
      ...
      this.logger.warn(`[MemoryCache] Deserialization failed for key '${key}': ${(error as Error).message}`);
      delete this.cache[normalizedKey];
      delete this.meta[normalizedKey];
      this._recordStat('misses');
      return null;
  ...
  protected override async _del(key: string): Promise<boolean> {
    const normalizedKey = this._normalizeKey(key);
  ...
  protected override async _clear(prefix?: string): Promise<boolean> {
    if (!prefix) {
      this.cache = {};
      this.meta = {};
      this.currentMemoryBytes = 0;
