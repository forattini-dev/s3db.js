...
export const PLUGIN_DEPENDENCIES: Record<string, PluginDefinition> = {
...
async function tryLoadPackage(packageName: string): Promise<PackageLoadResult> {
  try {
    ...
    return { installed: false, version: null, error: error as Error };
  }
}

export async function requirePluginDependency(
  pluginId: string,
  optionsOrCallerName: ValidationOptions | string = {}
): Promise<ValidationResult> {
  const options: ValidationOptions = typeof optionsOrCallerName === 'string'
  ...
  if (!valid && throwOnError) {
    ...
    const error = new Error(errorMsg) as PluginDependencyError;
...
export async function getPluginDependencyReport(): Promise<string> {
