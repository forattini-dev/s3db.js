/**
 * Nested Set Driver (Modified Preorder Tree Traversal)
 * 
 * Optimized for reads of entire subtrees.
 * Writes are expensive as they require rebalancing lft/rgt values.
 */

import { TasksRunner } from '../../../tasks/tasks-runner.class.js';
import { InvalidParentError, TreeConfigurationError, NodeNotFoundError, RootNodeError, TreeIntegrityError } from '../errors.js';
...
export class NestedSetDriver {
  ...
  async addChild(resourceName, parentId, data = {}) {
    ...
    return this.plugin._withLock(lockKey, async () => {
      // Re-fetch parent inside lock to ensure consistency
  ...
  async getNode(resourceName, nodeId) {
    ...
    try {
      ...
      if (err.code === 'NoSuchKey' || err.name === 'NoSuchKey' || err.message?.includes('No such key')) {
  ...
  async getAncestors(resourceName, nodeId, options = {}) {
    ...
    let ancestors = treeNodes.filter(n => {
      const left = n[this.config.leftField];
      const right = n[this.config.rightField];
  ...
  async moveSubtree(resourceName, nodeId, newParentId) {
    ...
    // Cannot move between different trees
    ...
    return this.plugin._withLock(lockKey, async () => {
      ...
      // Optimization: Parallel execution with TasksRunner
    ...
    return this.plugin._withLock(lockKey, async () => {
      ...
      if (children.length > 0) {
        if (promoteChildren) {
          ...
          // Optimization: Parallel updates with TasksRunner
          await TasksRunner.process(
            children,
            async (child) => {
  ...
  async deleteSubtree(resourceName, nodeId) {
    ...
    return this.plugin._withLock(lockKey, async () => {
      ...
      // Optimization: Parallel deletes with TasksRunner
      await TasksRunner.process(
        subtree,
        async (subtreeNode) => {
  ...
  async _shiftNodes(resourceName, fromValue, delta, treeId = null) {
    ...
    // Filter updates first to avoid processing unnecessary items
    ...
    // Optimization: Use TasksRunner for parallel patching
    await TasksRunner.process(
      nodesToUpdate,
      async (node) => {
        const left = node[this.config.leftField];
  ...
  async rebuildTree(resourceName, options = {}) {
    ...
    // If treeField is set but no treeId provided, rebuild all trees separately
    if (this.treeField && !treeId) {
      const resource = this.database.resources[resourceName];
      const allNodes = await resource.list({ limit: 10000 });
  ...
  async verifyTree(resourceName, options = {}) {
    ...
    // If treeField is set but no treeId provided, verify all trees separately
    if (this.treeField && !treeId) {
      const resource = this.database.resources[resourceName];
      const allNodes = await resource.list({ limit: 10000 });
      const treeIds = [...new Set(allNodes.map(n => n[this.treeField]).filter(Boolean))];
