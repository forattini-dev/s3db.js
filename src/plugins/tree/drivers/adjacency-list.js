...
export class AdjacencyListDriver {
  ...
  async addChild(resourceName, parentId, data = {}) {
    ...
    // Ensure parent exists and get its treeId
  ...
  async getRoots(resourceName, options = {}) {
    ...
    // Use partition if rootParentValue is a valid string (not null)
    ...
    // Fallback to scan if rootParentValue is null
  ...
  async moveSubtree(resourceName, nodeId, newParentId) {
    ...
    // Cycle detection is critical for Adjacency List
    ...
    // Just update the parent pointer - super fast!
  ...
  async deleteNode(resourceName, nodeId, options = {}) {
    ...
    if (children.length > 0) {
      if (promoteChildren) {
        ...
        // Optimization: Parallel update of children using TasksRunner
  ...
  async deleteSubtree(resourceName, nodeId) {
    ...
    // Optimization: Delete all in parallel using TasksRunner
    await TasksRunner.process(
      subtree,
      async (node) => {
  ...
  async rebuildTree(resourceName) {
    // Adjacency list doesn't strictly need rebuilding like Nested Set
    // But we could implement orphaned check here
