...
/**
 * WebhookReceiver - Process bounce/complaint/delivery notifications from email providers
 *
 * Supports:
 ...
 * Event types:
 * - bounce (hard/soft)
 * - complaint (abuse/fraud/general/not-spam)
...
export class WebhookReceiver {
  constructor(options = {}) {
    ...
    this.provider = options.provider || 'generic'; // sendgrid, aws-ses, mailgun, postmark
  ...
  async processWebhook(body, headers = {}) {
    try {
      ...
      throw new SMTPError(`Webhook processing error: ${err.message}`, {
        originalError: err,
   ...
   *
   * @param {string} eventType - bounce, complaint, delivery, open, click, etc.
   * @param {Function} handler - Async function to handle event
   */
  ...
  async _validateSignature(body, headers) {
    ...
    if (this.provider === 'sendgrid') {
      ...
      return this._validateMailgunSignature(body, headers);
    } else if (this.provider === 'postmark') {
  ...
  _validateSendGridSignature(body, headers) {
    ...
    // Reconstruct signed content: timestamp + request body
  ...
  /**
   * Validate AWS SES signature
   * @private
   */
  _validateAwsSesSignature(body) {
    // AWS SES uses SNS subscription verification
    if (body.Type === 'SubscriptionConfirmation') {
      return true; // Handle subscription confirmation separately
    }

    // For message notifications, validate Message signature
    ...
    // AWS uses RSA to sign with their certificate
    // In production, fetch and validate against AWS cert
   ...
   * @private
   */
  _validateMailgunSignature(body, headers) {
  ...
  _parseSendGridEvents(body) {
    ...
    return body.map((item) => {
      ...
      if (item.event === 'bounce') {
        event.type = 'bounce';
        event.bounceType = item.bounce_type || 'permanent'; // permanent, temporary
  ...
  /**
   * Parse AWS SES webhook events
   * @private
   */
  async _parseAwsSesEvents(body) {
    // AWS SES sends via SNS, so message is JSON-stringified
    ...
    // Handle open/click (if configured in SES)
    if (eventType === 'Open' || eventType === 'Click') {
      events.push({
        provider: 'aws-ses',
        type: eventType.toLowerCase(),
        messageId: mail.messageId,
        recipient: message.open?.recipient || message.click?.recipient || message.mail?.source,
  ...
  _parseMailgunEvents(body) {
    ...
    const timestamp = eventData.timestamp || body.timestamp;
    ...
    if (body.Bounces) {
      for (const bounce of body.Bounces) {
        events.push({
          ...
          timestamp: new Date(bounce.BouncedAt).getTime() / 1000,
          rawEvent: bounce
        });
      }
    }
    ...
    if (body.Complaints) {
      for (const complaint of body.Complaints) {
        events.push({
          ...
          timestamp: new Date(complaint.ComplainedAt).getTime() / 1000,
          rawEvent: complaint
        });
      }
    }
    ...
    if (body.Deliveries) {
      for (const delivery of body.Deliveries) {
        events.push({
          ...
          timestamp: new Date(delivery.DeliveredAt).getTime() / 1000,
          rawEvent: delivery
        });
      }
    }
  ...
  _parseGenericEvents(body) {
    // Generic format: { events: [ { type, recipient, messageId, ... } ] }
    if (Array.isArray(body.events)) {
      return body.events.map((evt) => ({
        provider: 'generic',
        type: evt.type,
