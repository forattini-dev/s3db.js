/**
 * Tech Detector - Fingerprints technologies used on web pages
 *
 * Detects:
 * - Frontend frameworks (React, Vue, Angular, Svelte, etc.)
 * - Analytics services (Google Analytics, Amplitude, Mixpanel, Hotjar, etc.)
 * - Marketing pixels (Facebook, LinkedIn, Google Ads, etc.)
 * - CDN providers (Cloudflare, CloudFront, Akamai, etc.)
 * - Web servers (nginx, Apache, IIS, etc.)
 * - CMS platforms (WordPress, Shopify, Drupal, etc.)
 * - JavaScript libraries (jQuery, Lodash, D3, etc.)
 */
export class TechDetector {
  ...
  /**
   * Detect web servers (via patterns in HTML)
   * @private
   */
  ...
  /**
   * Detect category of technologies
   * @private
   */
  ...
  _isDetected(html, signature) {
    // Check indicators (case-insensitive strings)
    ...
    // Check headers (look in meta tags or script tags)
