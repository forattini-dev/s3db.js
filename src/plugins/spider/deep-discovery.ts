...
import { CrawlContext, type HttpClientConfig } from './crawl-context.js';
...
export class DeepDiscovery {
  private config: Required<DeepDiscoveryConfig>;
  private _context: CrawlContext | null;
  private _httpClient: any | null;
  private discovered: DiscoveredData;
  private stats: DiscoveryStats;
  private crawlerCompatibility: CrawlerCompatibility;
   ...
   */
  async discover(baseUrl: string, options: DiscoveryOptions = {}): Promise<DiscoveryReport> {
    const opts = {
   ...
   */
  private async _analyzeRobotsTxt(domain: string): Promise<void> {
    try {
   ...
   */
  private async _discoverSitemaps(domain: string): Promise<void> {
    const variants = [
      // Standard locations
      '/sitemap.xml',
      '/sitemap_index.xml',
   ...
   */
  private async _discoverFeeds(domain: string): Promise<void> {
    const feedPaths = [
      // Generic feeds
      '/feed',
      '/feeds',
   ...
   */
  private async _detectPlatform(domain: string): Promise<void> {
    const detections: DiscoveredPlatform[] = [];

    // E-commerce platforms
    const ecommercePaths: Record<string, string[]> = {
    ...
    // CMS platforms
    const cmsPaths: Record<string, string[]> = {
   ...
   */
  private async _discoverAPIs(domain: string): Promise<void> {
    const apiPaths = [
      // Generic REST
      '/api',
      '/api/v1',
   ...
   */
  private async _discoverStaticFiles(domain: string): Promise<void> {
    const staticPaths = [
      // Configs
      '/config.json',
      '/settings.json',
   ...
   */
  private async _checkSubdomains(host: string): Promise<void> {
    const commonSubdomains = ['www', 'blog', 'news', 'shop', 'store', 'api', 'cdn', 'static', 'assets', 'media'];
    const protocol = 'https://';

    const subdomainSitemaps: DiscoveredSubdomain[] = [];
   ...
   */
  private async _probeUrls(domain: string, paths: string[]): Promise<ProbeResult[]> {
   ...
   */
  private async _getHttpClient(): Promise<any> {
    if (!this._httpClient) {
   ...
   */
  private async _getContentType(url: string): Promise<string> {
    if (this.config.fetcher) {
      try {
        const result = await this.config.fetcher(url);
   ...
   */
  private _looksLikeAPI(path: string): boolean {
    const apiPatterns = [
      /\/api\//i,
      /\/rest\//i,
      /\/graphql/i,
   ...
   */
  private _detectSitemapType(url: string): string {
    ...
    // Localized
    if (/(?:\/(en|pt|es|fr|de|it|ja|zh|ko)\/|sitemap-(?:en|pt|es|fr|de|it|ja|zh|ko)\.xml)/.test(urlLower)) {
      return 'localized';
    }
   ...
   */
  private _getSitemapPriority(url: string, type: string): number {
   ...
   * Detect feed type
   */
  private _detectFeedType(url: string, contentType: string | null): string {
   ...
   * Detect API type
   */
  private _detectAPIType(url: string, contentType: string | null): string {
   ...
   */
  private async _analyzeSitemapContent(url: string): Promise<Record<string, any>> {
    try {
   ...
   */
  private _scoreSitemapCompatibility(type: string, analysis: any): void {
    // Google scoring
    if (type === 'google-news') {
   ...
   */
  private _calculateCrawlerScores(): void {
    const keys = Object.keys(this.crawlerCompatibility) as (keyof CrawlerCompatibility)[];
    for (const crawler of keys) {
      // DuckDuckGo is derived, skip loop for it? No, loop runs for all keys.
      if (crawler === 'duckduckgo') continue;
   ...
   */
  private _estimateCrawlBudget(): CrawlBudget {
   ...
   */
  private _generateReport(domain: string): DiscoveryReport {
    ...
    return {
      ...
      stats: { ...this.stats },
      discovered: {
        sitemaps: this.discovered.sitemaps.sort((a, b) => (b.priority || 0) - (a.priority || 0)),
        feeds: this.discovered.feeds,
        apis: this.discovered.apis,
        staticFiles: this.discovered.staticFiles,
        frameworks: this.discovered.frameworks,
      ...
      summary: {
        ...
        successRate: ((this.stats.urlsFound / (this.stats.urlsProbed || 1)) * 100).toFixed(2) + '%'
      }
    };
  }
