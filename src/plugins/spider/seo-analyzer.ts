...
export interface AssetsAnalysis {
  stylesheets: Array<{ href: string; media?: string; type: string }>;
  scripts: Array<{ src: string; async?: boolean; defer?: boolean; type: string }>;
  images: Array<{ src: string | null; alt?: string; width?: string | null; height?: string | null }>;
  videos: Array<{ sources: Array<{ src: string | null; type: string | null }>; poster?: string | null; controls?: boolean }>;
  audios: Array<{ sources: Array<{ src: string | null; type: string | null }>; controls?: boolean }>;
  summary: {
...
export interface OnPageSEOAnalysis {
  title: { text: string; length: number; hasKeyword: boolean; quality: string } | null;
  h1: { count: number; texts: Array<{ text: string; quality: string }>; issue: string | null } | null;
  headingStructure: { total: number; byLevel: Record<string, number>; hierarchy: string | null; issue: string | null } | null;
  paragraphs: { count: number; avgLength: number; quality: { total: number; tooShort: number; readability: string }; issue: string | null } | null;
  lists: { count: number; unordered: number; ordered: number; totalItems: number } | null;
  images: { count: number; withAlt: number; withoutAlt: number; images: Array<{ src: string | null; alt: string | null; hasAlt: boolean; width: string | null; height: string | null }> } | null;
  contentMetrics?: {
    ...
    quality: string;
    detectedContentContainers: Array<{ selector: string; wordCount: number; matchType: string }>;
    suggestions: string[];
  } | null;
  url?: string | null;
...
export interface AccessibilityAnalysis {
  langAttribute: { present: boolean; value: string | null; issue: string | null } | null;
  headingStructure: { count: number; startsWithH1: boolean; properlySorted: boolean; issue: string | null } | null;
  altText: { total: number; withAlt: number; withoutAlt: number; percentage: number; issue: string | null } | null;
  formLabels: { inputs: number; labels: number; inputsWithLabels: number; issue: string | null } | null;
  semanticHTML: { elements: Record<string, number>; score: number; issue: string | null } | null;
  contrastRatios: unknown | null;
  ariaLabels: { total: number; withAriaLabel: number; withRole: number } | null;
  keyboardNavigation: { focusableElements: number; withTabindex: number; hasSkipLink: boolean } | null;
  recommendations: string[];
}

export interface InternalLinksAnalysis {
  total: number;
  sameDomain: { count: number; links: LinkAnalysis[] };
  subdomains: { count: number; links: LinkAnalysis[]; list: string[] };
  external: { count: number; links: LinkAnalysis[]; domains: Record<string, number> };
  orphaned: number;
  anchorTextQuality: { total: number; descriptive: number; poor: number; examples: string[] } | null;
  topicalClusters: { clusters: string[]; strength: number[]; recommendation: string | null } | null;
  recommendations: string[];
  referralAttributes?: {
    total: number;
    nofollow: number;
...
export class SEOAnalyzer {
   ...
   */
  private _analyzeOnPageSEO(doc: Document, html: string): OnPageSEOAnalysis {
   ...
   */
  private _analyzeContentLength(doc: Document, html: string): any {
    const analysis: any = {
      totalWordCount: 0,
   ...
   */
  private _estimateMainContent(doc: Document, html: string, totalWords: number): number {
    const paragraphs = doc.querySelectorAll('p');
    const lists = doc.querySelectorAll('ul, ol');
   ...
   */
  private _getMatchType(selector: string): string {
   ...
   */
  private _analyzeAccessibility(doc: Document, html: string): AccessibilityAnalysis {
   ...
   */
  private _analyzeInternalLinks(doc: Document, baseUrl: string): InternalLinksAnalysis {
    ...
    const samedomainLinks: LinkAnalysis[] = [];
    const subdomainLinks: LinkAnalysis[] = [];
    const externalLinks: LinkAnalysis[] = [];
    ...
    analysis.orphaned = anchorLinks.filter((id) => id && !headingIds.includes(id)).length;

    // Topical clusters recommendation
    const internalDomainsCount: Record<string, number> = {};
   ...
   */
  private _analyzeKeywordOptimization(doc: Document, html: string, metaTags: Record<string, string> | null): KeywordOptimizationAnalysis | null {
    const analysis: KeywordOptimizationAnalysis = {
      primaryKeyword: null,
      secondaryKeywords: [],
      keywordDensity: null,
      inTitle: false,
    ...
    if (keywords.length > 0) {
      analysis.primaryKeyword = keywords[0] ?? null;

      // Check keyword placement
      analysis.inTitle = analysis.primaryKeyword ? bodyText.includes(analysis.primaryKeyword) : false;
      analysis.inH1 = analysis.primaryKeyword ? h1Text.includes(analysis.primaryKeyword) : false;
      analysis.inFirstParagraph = analysis.primaryKeyword ? firstParagraph.includes(analysis.primaryKeyword) : false;
   ...
   * Calculate overall SEO score
   */
  private _calculateSEOScore(result: AnalysisResult): SEOScore {
    let score = 0;
    let maxScore = 0;
    ...
    if (result.onPageSEO) {
      ...
      if ((result.onPageSEO.headingStructure?.total || 0) > 0) score += 10;
      if ((result.onPageSEO.contentMetrics?.mainContentWordCount || 0) > 300) score += 10;
    }
    ...
    if (result.accessibility) {
      ...
      if ((result.accessibility.altText?.percentage || 0) > 80) score += 5;
      if (result.accessibility.headingStructure?.properlySorted) score += 5;
      if ((result.accessibility.semanticHTML?.score || 0) > 0) score += 5;
    }

    // Internal links (10 points)
    maxScore += 10;
    if (result.internalLinks) {
      if ((result.internalLinks.sameDomain?.count || 0) >= 5) score += 10;
    }
  ...
  private _evaluateTitleQuality(title: string): string {
    if (title.length < 30) return 'short';
    if (title.length > 60) return 'long';
    return 'optimal';
  }

  private _evaluateHeadingQuality(heading: string): string {
    if (heading.length < 10) return 'too-short';
    if (heading.length > 100) return 'too-long';
    return 'good';
  }

  private _countHeadingsByLevel(headings: NodeList): Record<string, number> {
    const counts: Record<string, number> = {};
    for (const heading of headings) {
      const level = (heading as Element).tagName;
      counts[level] = (counts[level] || 0) + 1;
    }
    return counts;
  }

  private _analyzeHeadingHierarchy(headings: NodeList): string | null {
    if (headings.length === 0) return null;
    const first = (headings[0] as Element).tagName;
    return first === 'H1' ? 'proper' : 'improper';
  }

  private _findHeadingIssues(headings: NodeList): string | null {
    if (headings.length === 0) return 'No headings found';
    const h1Count = Array.from(headings).filter((h) => (h as Element).tagName === 'H1').length;
    if (h1Count !== 1) return `Found ${h1Count} H1 tags (should be 1)`;
    return null;
  }

  private _calculateAverageLength(paragraphs: NodeList): number {
    if (paragraphs.length === 0) return 0;
    const totalLength = Array.from(paragraphs).reduce((sum, p) => sum + (p.textContent?.length || 0), 0);
    return Math.round(totalLength / paragraphs.length);
  }

  private _analyzeParagraphQuality(paragraphs: NodeList): { total: number; tooShort: number; readability: string } {
    const shortParagraphs = Array.from(paragraphs).filter((p) => (p.textContent?.length || 0) < 50).length;
    return {
  ...
  private _evaluateAnchorQuality(anchorText: string): string {
    const poorTexts = ['click here', 'read more', 'more', 'link', ''];
    return !poorTexts.includes(anchorText.toLowerCase()) ? 'descriptive' : 'generic';
  }

  private _checkHeadingOrder(headings: NodeList): boolean {
    if (headings.length === 0) return true;
    if ((headings[0] as Element).tagName !== 'H1') return false;

    let lastLevel = 1;
    for (const heading of headings) {
      const level = parseInt((heading as Element).tagName[1]!);
  ...
  private _countInputsWithLabels(inputs: NodeList, labels: NodeList): number {
    let count = 0;
    const labelTexts = Array.from(labels).map((l) => (l as HTMLLabelElement).htmlFor);

    for (const input of inputs) {
      if ((input as Element).id && labelTexts.includes((input as Element).id)) {
        count++;
      } else if ((input as Element).closest('label')) {
        count++;
      }
    }
    return count;
  ...
  private _extractMetaTags(doc: Document): Record<string, string> | null {
  ...
  private _extractMetaTagsRegex(html: string): Record<string, string> | null {
    const metaTags: Record<string, string> = {};
  ...
  private _extractOpenGraph(doc: Document): Record<string, string> | null {
  ...
  private _extractOpenGraphRegex(html: string): Record<string, string> | null {
    const og: Record<string, string> = {};
  ...
  private _extractTwitterCard(doc: Document): Record<string, string> | null {
  ...
  private _extractTwitterCardRegex(html: string): Record<string, string> | null {
    const twitter: Record<string, string> = {};
  ...
  private _extractCanonical(doc: Document): string | null {
    const link = doc.querySelector('link[rel="canonical"]');
    return link ? link.getAttribute('href') : null;
  }

  private _extractCanonicalRegex(html: string): string | null {
    const match = html.match(/<link\s+rel=["']canonical["']\s+href=["']([^"']+)["']/i);
    return match ? match[1]! : null;
  }

  private _extractAlternates(doc: Document): Array<{ hreflang: string; href: string }> {
  ...
  private _extractAlternatesRegex(html: string): Array<{ hreflang: string; href: string }> {
    const alternates: Array<{ hreflang: string; href: string }> = [];
  ...
  private _extractAssets(doc: Document, baseUrl: string): AssetsAnalysis {
    const assets: AssetsAnalysis = {
      stylesheets: [],
      scripts: [],
      images: [],
      videos: [],
  ...
  private _extractAssetsRegex(html: string, baseUrl: string): AssetsAnalysis {
    const assets: AssetsAnalysis = {
      stylesheets: [],
      scripts: [],
      images: [],
    ...
    while ((match = imgRegex.exec(html)) !== null) {
      assets.images.push({ src: match[1] ?? null });
    }
  ...
  private _extractImageFormats(images: any[]): Record<string, number> {
  ...
  private _extractVideoFormats(videos: any[]): Record<string, number> {
    const formats: Record<string, number> = {};
    for (const video of videos) {
      if (video.sources) {
  ...
  private _extractAudioFormats(audios: any[]): Record<string, number> {
    const formats: Record<string, number> = {};
    for (const audio of audios) {
      if (audio.sources) {
