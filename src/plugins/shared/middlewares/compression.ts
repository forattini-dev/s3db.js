...
export function createCompressionMiddleware(
  ...
  return async function(this: CompressionContext | undefined, c: Context, next: Next): Promise<void> {
    await next();
    ...
    try {
      ...
      const logger = context?.logger || (this as CompressionContext | undefined)?.logger;
      if (logger && (logLevel === 'debug' || logLevel === 'trace')) {
        logger.error({ error: (err as Error).message }, '[Compression] Error');
      }
    }
  };
}
