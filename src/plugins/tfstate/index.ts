...
export class TfStatePlugin extends Plugin {
  ...
  override async onInstall(): Promise<void> {
    ...
    {
      const [created, createErr, resource] = await tryFn(() => (this as any).database.createResource({
      ...
      if (created) {
        ...
        this.lineagesResource = (this as any).database.resources?.[this.lineagesName];
    ...
    // Dedicated to tracking state file metadata with SHA256 hash for deduplication
    {
      ...
      if (created) {
        ...
        this.stateFilesResource = (this as any).database.resources?.[this.stateFilesName];
    ...
    // Store extracted resources with foreign key to state files
    {
      ...
      if (created) {
        ...
        this.resource = (this as any).database.resources?.[this.resourceName];
        if (!this.resource) {
          throw createErr;
        }
      }
    ...
    if (this.trackDiffs) {
      ...
      if (created) {
        ...
        this.diffsResource = (this as any).database.resources?.[this.diffsName];
   ...
   */
  async importStatesGlob(pattern: string, options: any = {}): Promise<any> {
   ...
   * @private
   */
  async _findFilesGlob(pattern: string): Promise<string[]> {
    const files: string[] = [];
   ...
   */
  async importStateFromS3(bucket: string, key: string, options: any = {}): Promise<any> {
    const startTime = Date.now();
    ...
    try {
      ...
      const client = options.client || (this as any).database.client;
      ...
      this.stats.resourcesExtracted += ((resources as any).totalExtracted || resources.length);
      ...
      const result = {
        ...
        lineage: state.lineage,
        terraformVersion: state.terraform_version,
        resourcesExtracted: ((resources as any).totalExtracted || resources.length),
   ...
   */
  async importStatesFromS3Glob(bucket: string, pattern: string, options: any = {}): Promise<any> {
    const startTime = Date.now();
    const client = options.client || (this as any).database.client;
    const parallelism = options.parallelism || 5;
    ...
    try {
      ...
      const matchingObjects = allObjects.filter((obj: any) => {
   ...
   * @private
   */
  async _ensureLineage(lineageUuid: string, stateMeta: any): Promise<any> {
    if (!lineageUuid) {
   ...
   */
  async importState(filePath: string): Promise<any> {
    const startTime = Date.now();
   ...
   * @private
   */
  async _readStateFile(filePath: string): Promise<any> {
   ...
   * @private
   */
  async _extractResources(state: any, filePath: string, stateFileId: string, lineageId: string | null): Promise<any[]> {
    const resources: any[] = [];
    ...
    (resources as any).totalExtracted = totalExtracted;
   ...
   * @private
   */
  async _calculateDiff(currentState: any, lineageId: string, currentStateFileId: string): Promise<any> {
   ...
   * @private
   */
  async _computeDiff(oldSerial: number, newSerial: number, lineageId: string): Promise<any> {
    ...
    const oldMap = new Map(oldResources.map((r: any) => [r.resourceAddress, r]));
    const newMap = new Map(newResources.map((r: any) => [r.resourceAddress, r]));
    ...
    for (const [address, newResource] of newMap) {
      if (!oldMap.has(address)) {
        added.push({
          address,
          type: (newResource as any).resourceType,
          name: (newResource as any).resourceName
        });
      } else {
        // Check if modified (simple attribute comparison)
        const oldResource = oldMap.get(address);
        if (JSON.stringify((oldResource as any).attributes) !== JSON.stringify((newResource as any).attributes)) {
          modified.push({
            address,
            type: (newResource as any).resourceType,
            name: (newResource as any).resourceName,
            changes: this._computeAttributeChanges((oldResource as any).attributes, (newResource as any).attributes)
          });
        }
      }
    }
    ...
    for (const [address, oldResource] of oldMap) {
      if (!newMap.has(address)) {
        deleted.push({
          address,
          type: (oldResource as any).resourceType,
          name: (oldResource as any).resourceName
        });
      }
    }
   ...
   * @private
   */
  async _saveDiff(diff: any, lineageId: string, newStateFileId: string): Promise<any> {
    const diffRecord = {
      id: idGenerator(),
   ...
   * @private
   */
  async _insertResources(resources: any[]): Promise<any[]> {
    if (resources.length === 0) return [];

    const inserted: any[] = [];
    const parallelism = (this as any).database.parallelism || 10;
   ...
   * @private
   */
  async _setupCronMonitoring(): Promise<void> {
    if (!this.driver) {
    ...
    const cron = (cronModule as any).default;
   ...
   * @private
   */
  async _monitorStateFiles(): Promise<any> {
   ...
   * @private
   */
  async _setupFileWatchers(): Promise<void> {
          ...
          for await (const event of watcher) {
            if (event.eventType === 'change' && event.filename && event.filename.endsWith('.tfstate')) {
              ...
              try {
                ...
                this.stats.errors++;
                this.logger.error(`[TfStatePlugin] Auto-import failed: ${(error as Error).message}`);
   ...
   */
  async exportState(options: any = {}): Promise<any> {
    const {
      serial,
      resourceTypes,
    ...
    if (partitionName) {
      ...
      if (resourceTypes && resourceTypes.length > 0) {
        resources = resources.filter((r: any) => resourceTypes.includes(r.resourceType));
      }
    } else {
      ...
      resources = allResources.filter((r: any) => {
    ...
    for (const resourceGroup of resourceMap.values()) {
      resourceGroup.instances.sort((a: any, b: any) => {
   ...
   */
  async exportStateToFile(filePath: string, options: any = {}): Promise<any> {
    ...
    return {
      filePath,
      serial: state.serial,
      resourceCount: state.resources.reduce((sum: number, r: any) => sum + r.instances.length, 0),
   ...
   * Export state to S3
   */
  async exportStateToS3(bucket: string, key: string, options: any = {}): Promise<any> {
    const state = await this.exportState(options);
    const client = options.client || (this as any).database.client;
   ...
   */
  async compareStates(sourceFile: string, oldSerial: number, newSerial: number): Promise<any> {
    if (!this.diffsResource) {
      throw new TfStateError('Diff tracking is not enabled for this plugin');
    }
   ...
   */
  async triggerMonitoring(): Promise<any> {
    if (!this.driver) {
   ...
   */
  async getResourcesByProvider(provider: string): Promise<any[]> {
    return await this.resource.listPartition({
   ...
   */
  async getResourcesByProviderAndType(provider: string, type: string): Promise<any[]> {
    return await this.resource.listPartition({
   ...
   */
  async getDiff(sourceFile: string, oldSerial: number, newSerial: number): Promise<any> {
   ...
   */
  async getStatsByProvider(): Promise<Record<string, number>> {
    const allResources = await this.resource.list({ limit: 100000 });
  ...
  _findPartitionByField(resource: any, fieldName: string): string | null {
    ...
    for (const [partitionName, partitionConfig] of Object.entries(resource.config.partitions) as any) {
  ...
  async getStats(): Promise<any> {
    ...
    const latestSerial = stateFiles.length > 0
      ? Math.max(...stateFiles.map((sf: any) => sf.serial))
      : null;
