...
type ReckerExtractLinks = (html: string, baseUrl?: string) => Array<{
  href: string;
  text?: string;
  rel?: string;
}>;

type ReckerParse = (html: string) => {
  querySelector(selector: string): { textContent?: string } | null;
  querySelectorAll(selector: string): Array<{ getAttribute(name: string): string | null; textContent?: string }>;
};
...
export class ReckerScrapeStage {
  ...
  private extractLinks: ReckerExtractLinks | null = null;
  private parse: ReckerParse | null = null;
  private fallbackStage: import('./google-dorks-stage.js').GoogleDorksStage | null = null;
  ...
  private async _checkReckerAvailability(): Promise<boolean> {
    ...
    try {
      ...
      this.extractLinks = scrapeModule.extractLinks as unknown as ReckerExtractLinks;
      this.parse = scrapeModule.parseHtmlSync as unknown as ReckerParse;
  ...
  private async _getFallbackStage(): Promise<import('./google-dorks-stage.js').GoogleDorksStage> {
  ...
  async execute(target: Target, options: ReckerScrapeFeatureConfig = {}): Promise<ReckerScrapeResult> {
    ...
    const result: Omit<ReckerScrapeResult, '_individual' | '_aggregated'> = {
      status: 'ok',
      domain,
      companyName,
      categories: {
  ...
  private async _performSearch(query: string, options: ReckerScrapeFeatureConfig = {}): Promise<string[]> {
  ...
  private async _searchGitHub(domain: string, companyName: string, options: ReckerScrapeFeatureConfig): Promise<CategoryResult> {
    const queries = [
  ...
  private async _searchPastebin(domain: string, companyName: string, options: ReckerScrapeFeatureConfig): Promise<CategoryResult> {
    const queries = [
  ...
  private async _searchLinkedIn(domain: string, companyName: string, options: ReckerScrapeFeatureConfig): Promise<CategoryResult> {
    const queries = [
  ...
  private async _searchDocuments(domain: string, options: ReckerScrapeFeatureConfig): Promise<CategoryResult> {
  ...
  private async _searchSubdomains(domain: string, options: ReckerScrapeFeatureConfig): Promise<CategoryResult> {
  ...
  private async _searchLoginPages(domain: string, options: ReckerScrapeFeatureConfig): Promise<CategoryResult> {
    const queries = [
  ...
  private async _searchConfigs(domain: string, options: ReckerScrapeFeatureConfig): Promise<CategoryResult> {
    const queries = [
      `site:${domain} ext:env`,
  ...
  private async _searchErrors(domain: string, options: ReckerScrapeFeatureConfig): Promise<CategoryResult> {
    const queries = [
  ...
  private _deduplicateResults(results: SearchResultItem[]): SearchResultItem[] {
  ...
  private async _executeFallback(target: Target, options: ReckerScrapeFeatureConfig): Promise<ReckerScrapeResult> {
    const fallback = await this._getFallbackStage();
    const fallbackResult = await fallback.execute(target, options);

    return fallbackResult as ReckerScrapeResult;
  }
