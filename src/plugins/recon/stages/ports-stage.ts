...
export class PortsStage {
  ...
  private _normalizePorts(data: any): PortEntry[] {
    ...
    if (Array.isArray(data)) {
      return data.map(port => this._normalizePortEntry(port)).filter((p): p is PortEntry => p !== null);
    }

    if (data.ports) {
      return data.ports.map((port: any) => this._normalizePortEntry(port)).filter((p: PortEntry | null): p is PortEntry => p !== null);
    }

    if (data.open_ports) {
      return data.open_ports.map((port: any) => this._normalizePortEntry(port)).filter((p: PortEntry | null): p is PortEntry => p !== null);
    }

    return [];
  }

  private _normalizePortEntry(entry: any): PortEntry | null {
  ...
  private _parseRawOutput(raw: string): PortEntry[] {
