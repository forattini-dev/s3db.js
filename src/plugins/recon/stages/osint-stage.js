...
export class OsintStage {
  ...
  async _enumerateUsernames(username, config) {
    const rbResult = await this.commandRunner.runRedBlue(
      ...
      username,
      {
        ...
        flags: config.maxSites ? ['--max-sites', String(config.maxSites)] : []
      }
    );
  ...
  _normalizeProfiles(data, username) {
    ...
    if (Array.isArray(data.profiles)) {
      profiles.push(...data.profiles.map(p => this._normalizeProfile(p, username)));
    } else if (Array.isArray(data.results)) {
      profiles.push(...data.results.map(p => this._normalizeProfile(p, username)));
    } else if (Array.isArray(data)) {
      profiles.push(...data.map(p => this._normalizeProfile(p, username)));
    }

    return this._deduplicateProfiles(profiles.filter(Boolean));
  }
