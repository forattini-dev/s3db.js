...
export class TargetManager {
  ...
  async add(targetInput: string, options: TargetOptions = {}): Promise<TargetRecord> {
  ...
  async remove(targetInput: string): Promise<{ targetId: string; removed: boolean }> {
    const normalized = this._normalizeTarget(targetInput);
    const targetId = normalized.host;
  ...
  async update(targetInput: string, updates: Partial<TargetRecord>): Promise<TargetRecord> {
    const normalized = this._normalizeTarget(targetInput);
    const targetId = normalized.host;
  ...
  async list(options: ListOptions = {}): Promise<TargetRecord[]> {
    ...
    try {
      const targetsResource = await this._getResource();
      let targets: TargetRecord[] = await targetsResource.list({ limit });
  ...
  async get(targetInput: string): Promise<TargetRecord | null> {
    const normalized = this._normalizeTarget(targetInput);
    const targetId = normalized.host;
  ...
  async updateScanMetadata(targetId: string, report: Report): Promise<void> {
    try {
  ...
  private async _getResource(): Promise<any> {
  ...
  private _normalizeTarget(targetInput: string): NormalizedTarget {
  ...
  private _defaultPortForProtocol(protocol: string | null): number | null {
    switch (protocol) {
      case 'http':
        return 80;
      case 'https':
  ...
  private _normalizeConfigTargets(): TargetRecord[] {
