...
import type { Bracket, BracketMatch, Group, ScheduleRound } from './types.js';
...
export interface SwissPairingOptions {
  participants: string[];
  standings: { participantId: string; points: number }[];
  completedPairings: { participant1Id: string; participant2Id: string }[];
}
...
export function generateSwissPairing(options: SwissPairingOptions): { participant1Id: string; participant2Id: string | null }[] {
  const { participants, standings, completedPairings } = options;

  const sorted = [...standings].sort((a, b) => b.points - a.points);
  const paired = new Set<string>();
  const pairings: { participant1Id: string; participant2Id: string | null }[] = [];
  ...
  for (const standing of sorted) {
    if (paired.has(standing.participantId)) continue;

    let opponent: string | null = null;

    for (const candidate of sorted) {
      if (candidate.participantId === standing.participantId) continue;
      if (paired.has(candidate.participantId)) continue;

      const pairKey1 = `${standing.participantId}:${candidate.participantId}`;
      if (alreadyPlayed.has(pairKey1)) continue;
...
export function generateGSLBracket(options: GSLOptions): Group {
...
function fillWithByes(participants: string[], size: number): (string | null)[] {
...
function processByeAdvancements(matches: BracketMatch[][]): void {
...
function calculateLoserDropRound(winnersRound: number, totalWinnersRounds: number): number | null {
  if (winnersRound === totalWinnersRounds) return null;
