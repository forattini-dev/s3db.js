 ...
 * Features Winners Bracket, Losers Bracket, and Grand Finals
 */
...
export class DoubleEliminationFormat extends BaseFormat {
  ...
  onMatchComplete(bracket, completedMatch) {
    ...
    // Determine which bracket the match is in
    if (matchId.startsWith('WR')) {
      ...
      // Grand finals reset
      bracket.grandFinalsReset = null; // Tournament complete
    }
  ...
  _processGrandFinals(bracket, match, newMatches) {
    ...
    if (bracket.grandFinalsReset && match.winnerId === bracket.grandFinals.participant2Id) {
      // Losers bracket winner won - play reset
      newMatches.push({
        ...this.createMatchTemplate({
          phase: 'grand-finals-reset',
          round: 2,
          matchNumber: 1,
          participant1Id: bracket.grandFinals.participant1Id, // Original winners bracket winner
          participant2Id: bracket.grandFinals.participant2Id, // Losers bracket winner
  ...
  _advanceInWinners(bracket, nextMatchId, participantId, newMatches) {
    const [, roundStr, matchStr] = nextMatchId.match(/WR(\d+)M(\d+)/) || [];
    if (!roundStr) return;
  ...
  _advanceInLosers(bracket, nextMatchId, participantId, newMatches) {
    const [, roundStr, matchStr] = nextMatchId.match(/LR(\d+)M(\d+)/) || [];
    if (!roundStr) return;
  ...
  _dropToLosers(bracket, loserMatchId, participantId, newMatches) {
    const [, roundStr, matchStr] = loserMatchId.match(/LR(\d+)M(\d+)/) || [];
    if (!roundStr) return;
  ...
  getStandings(bracket, matches) {
    ...
    // Process losers bracket eliminations (3rd-4th, 5th-6th, etc.)
  ...
  isComplete(bracket, matches) {
    ...
    // Check if reset was needed and completed
  ...
  getCurrentRound(bracket, matches) {
    // Find the highest incomplete round
    const allMatches = [
