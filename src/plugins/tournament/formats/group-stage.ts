...
interface GroupBracket {
  ...
  style: 'round-robin' | 'gsl';
  participants: string[];
  schedule?: { matches: BracketMatch[][] };
  bracket?: Group;
  standings?: Standing[];
  advancing?: { participantId: string; seed: number }[];
  eliminated?: string[];
  complete: boolean;
}

interface GSLMatch extends BracketMatch {
  type?: 'opening' | 'winners' | 'losers' | 'decider';
  winnerNextMatch?: string;
...
interface GroupStageBracket {
  ...
  groups: GroupBracket[];
  advanceCount: number;
  advancing: { participantId: string; seed: number; groupId: string }[];
  currentGroup: number;
}

export class GroupStageFormat extends BaseFormat {
  declare config: GroupStageConfig;
  ...
  static override get defaultConfig(): GroupStageConfig {
    return {
      groupCount: 4,
  ...
  override validate(participants: string[], config: GroupStageConfig): ValidationResult {
    const errors: string[] = [];
  ...
  override generateBracket(participants: string[], config: FormatConfig): Bracket {
    const gsConfig = config as GroupStageConfig;
    const groupCount = (gsConfig.groupCount as number) || 4;
    const perGroup = (gsConfig.participantsPerGroup as number) || Math.ceil(participants.length / groupCount);
    ...
    const groupBrackets = groups.map((groupParticipants, index) => {
      ...
      if (gsConfig.style === 'gsl') {
        ...
        return {
          groupId,
          style: 'round-robin' as const,
          participants: groupParticipants,
          schedule: generateRoundRobinSchedule(groupParticipants, {
            rounds: (gsConfig.rounds as number) || 1,
    ...
    return {
      ...
      groups: groupBrackets as unknown as Group[],
      advanceCount: (gsConfig.advanceCount as number) || 2,
      advancing: [],
      currentGroup: 0
    } as unknown as Bracket;
  }

  private _distributeSequential(participants: string[], groupCount: number): string[][] {
  ...
  getInitialMatches(bracket: Bracket): Match[] {
    const groupBracket = bracket as unknown as GroupStageBracket;
    const matches: Match[] = [];

    for (const group of groupBracket.groups) {
      if (group.style === 'gsl') {
        const gslMatches = group.bracket!.matches as GSLMatch[];
  ...
  onMatchComplete(bracket: Bracket, completedMatch: Match): OnMatchCompleteResult {
    ...
    if (!group) return { bracket: groupBracket as unknown as Bracket, newMatches };
    ...
    if (groupBracket.groups.every(g => g.complete)) {
      groupBracket.advancing = groupBracket.groups.flatMap(g =>
        (g.advancing || []).map(a => ({ ...a, groupId: g.groupId }))
      );
    }

    return { bracket: groupBracket as unknown as Bracket, newMatches };
  }

  private _processGSLMatch(bracket: GroupStageBracket, group: GroupBracket, match: Match, newMatches: Match[]): void {
    const gslMatches = group.bracket!.matches as GSLMatch[];
    const gslMatch = gslMatches.find(m =>
    ...
    if (gslMatch.type === 'opening') {
      const winnersMatch = gslMatches.find(m => m.id === 'WM') as GSLMatch;
      const losersMatch = gslMatches.find(m => m.id === 'LM') as GSLMatch;
      ...
      const deciderMatch = gslMatches.find(m => m.id === 'DM') as GSLMatch;
  ...
  private _processRoundRobinMatch(bracket: GroupStageBracket, group: GroupBracket, match: Match, newMatches: Match[]): void {
    ...
    if (allComplete) {
      ...
      const standings = calculateRoundRobinStandings(allMatches as unknown as Match[], bracket.config as GroupStageConfig);
  ...
  getStandings(bracket: Bracket, matches: Match[]): Standing[] {
    ...
    for (const group of groupBracket.groups) {
      if (group.style === 'round-robin') {
        ...
        const standings = calculateRoundRobinStandings(groupMatches, groupBracket.config as GroupStageConfig);

        for (const s of standings) {
          allStandings.push({ ...s, groupId: group.groupId } as Standing & { groupId: string });
        }
        ...
        for (const adv of group.advancing || []) {
          allStandings.push({
            ...
            rank: adv.seed,
            status: 'advanced'
          } as Standing & { groupId: string; status: string });
        }
  ...
  override getWinner(_bracket: Bracket, _matches: Match[]): string | null {
    return null;
  }

  getAdvancing(bracket: Bracket): { participantId: string; seed: number; groupId: string }[] {
    return (bracket as unknown as GroupStageBracket).advancing;
  }
