...
export function calculateRoundRobinStandings(matches: Match[], config: { pointsWin?: number; pointsDraw?: number; pointsLoss?: number } = {}): Standing[] {
  const { pointsWin = 3, pointsDraw = 1, pointsLoss = 0 } = config;
  const stats: Map<string, Standing> = new Map();
...
export function calculateEliminationStandings(matches: Match[], bracket: Bracket): Standing[] {
...
export function calculateSwissStandings(matches: Match[], participants: string[]): Standing[] {
  const stats: Map<string, Standing> = new Map();
...
function calculateBuchholz(stats: Map<string, Standing>, matches: Match[]): void {
  const opponents: Map<string, string[]> = new Map();
  ...
  for (const match of matches) {
    ...
    if (!opponents.has(p1)) opponents.set(p1, []);
    if (!opponents.has(p2)) opponents.set(p2, []);
...
function sortSwissStandings(standings: Standing[]): Standing[] {
...
export function calculateLadderRankings(rankings: LadderRanking[]): Standing[] {
  return rankings
...
export function calculateCircuitStandings(events: { results: { participantId: string; points: number }[] }[]): Standing[] {
...
export function applyHeadToHead(standings: Standing[], matches: Match[]): Standing[] {
...
function sortByHeadToHead(tied: Standing[], matches: Match[]): Standing[] {
  ...
  for (const match of matches) {
    ...
    if (!p1 || !p2 || !tiedIds.has(p1) || !tiedIds.has(p2)) continue;
...
function createStanding(participantId: string): Standing {
  return {
    participantId,
    points: 0,
    wins: 0,
...
function countWins(matches: Match[], participantId: string): number {
  return matches.filter(m => m.status === 'completed' && m.winnerId === participantId).length;
}

function countLosses(matches: Match[], participantId: string): number {
