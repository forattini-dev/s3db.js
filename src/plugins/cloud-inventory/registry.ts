...
const CLOUD_DRIVER_LAZY_LOADERS: Record<string, DriverLoader> = {
...
const CLOUD_DRIVERS: Map<string, DriverFactory> = new Map();
...
export function registerCloudDriver(name: string, factory: DriverFactory): void {
...
export async function createCloudDriver(name: string, options?: Partial<BaseCloudDriverOptions>): Promise<BaseCloudDriver> {
  if (CLOUD_DRIVERS.has(name)) {
    const resolvedOptions: BaseCloudDriverOptions = {
      driver: name,
      ...(options || {})
    };
...
export function listCloudDrivers(): string[] {
...
export function validateCloudDefinition(cloud: unknown): void {
  ...
  const { driver, credentials } = cloud as CloudDefinition;
...
class NoopDriver extends BaseCloudDriver {
  private sampleResources: CloudResource[];

  constructor(options: BaseCloudDriverOptions) {
    super(options);
    this.sampleResources = (options.config as { sampleResources?: CloudResource[] })?.sampleResources || [];
  }
