...
export class CodeSamplesGenerator {
  static generate(route: RouteMetadata, baseUrl: string = 'https://api.example.com'): CodeSamples {
  ...
  private static generateObjectExample(schema: OpenAPISchema): Record<string, unknown> {
  ...
  private static generateArrayExample(schema: OpenAPISchema): unknown[] {
  ...
  private static generateStringExample(schema: OpenAPISchema): string {
    if (schema.enum) return schema.enum[0] as string;
  ...
  private static generateNumberExample(schema: OpenAPISchema): number {
  ...
  private static generateCurl(method: string, url: string, body: unknown, route: RouteMetadata): string {
  ...
  private static generateNodeJS(method: string, url: string, body: unknown, route: RouteMetadata): string {
...
console.log(data);`;
  ...
  private static generateJavaScript(method: string, url: string, body: unknown, route: RouteMetadata): string {
    const hasBody = body && ['POST', 'PUT', 'PATCH'].includes(method);
    const hasAuth = route.guards && route.guards.length > 0;
  ...
  private static generatePython(method: string, url: string, body: unknown, route: RouteMetadata): string {
    const hasBody = body && ['POST', 'PUT', 'PATCH'].includes(method);
    const hasAuth = route.guards && route.guards.length > 0;
...
print(response.json())`;
  ...
  private static generatePHP(method: string, url: string, body: unknown, route: RouteMetadata): string {
    const hasBody = body && ['POST', 'PUT', 'PATCH'].includes(method);
    const hasAuth = route.guards && route.guards.length > 0;
...
?>`;
  ...
  private static generateGo(method: string, url: string, body: unknown, route: RouteMetadata): string {
    const hasBody = body && ['POST', 'PUT', 'PATCH'].includes(method);
    const hasAuth = route.guards && route.guards.length > 0;
...
func main() {
  ...
  private static generateValidationErrorExample(schema: OpenAPISchema): ErrorResponse['example'] {
    const errors: Array<{ field: string; message: string; type: string; expected?: string; actual?: string }> = [];
