...
export function getStatusFromError(err: Error | S3DBError): number {
...
export function errorHandler(err: Error | S3DBError, c: Context): Response {
  const status = getStatusFromError(err);
  const code = err.name || 'INTERNAL_ERROR';

  const details: ErrorDetails = {};

  const s3dbErr = err as S3DBError;
  if (s3dbErr.resource) details.resource = s3dbErr.resource;
  if (s3dbErr.bucket) details.bucket = s3dbErr.bucket;
  if (s3dbErr.key) details.key = s3dbErr.key;
  if (s3dbErr.operation) details.operation = s3dbErr.operation;
  if (s3dbErr.suggestion) details.suggestion = s3dbErr.suggestion;
  if (s3dbErr.availableResources) details.availableResources = s3dbErr.availableResources;
