...
export type GuardFunction = (
  ctx: RouteContext,
  deps: { db: unknown; resources: Record<string, unknown> }
...
export interface HealthCheckOptions {
  checker?: (ctx: RouteContext) => Promise<{ healthy?: boolean; checks?: Record<string, unknown> }>;
}
...
export class ApiApp {
  ...
  constructor({ db = null, resources = null }: ApiAppOptions = {}) {
    ...
    this.guards = new Map();
    this.middlewares = [];
    this.validator = new (Validator as unknown as new () => ValidatorInstance)();
    this.db = db;
    this.resources = resources || (db as { resources?: Record<string, unknown> })?.resources || {};
  ...
  route(methodOrPath: string, pathOrApp: string | Hono, options: RouteOptions = {}, handler?: RouteHandler): ApiApp {
    ...
    const path = pathOrApp as string;
  ...
  guard(name: string, guardFn: GuardFunction, { priority = DEFAULT_PRIORITY } = {}): ApiApp {
  ...
  group(basePath: string, options: GroupOptions = {}): GroupProxy {
    ...
    return {
      route: (method: string, path: string, routeOptions: RouteOptions, handler: RouteHandler) => {
      ...
      post: (path: string, options: RouteOptions | RouteHandler, handler?: RouteHandler) => {
        const fullPath = `${basePath}${path}`;
        if (typeof options === 'function') {
          handler = options;
          options = {};
      ...
      put: (path: string, options: RouteOptions | RouteHandler, handler?: RouteHandler) => {
        const fullPath = `${basePath}${path}`;
        if (typeof options === 'function') {
          handler = options;
          options = {};
      ...
      patch: (path: string, options: RouteOptions | RouteHandler, handler?: RouteHandler) => {
        const fullPath = `${basePath}${path}`;
        if (typeof options === 'function') {
          handler = options;
          options = {};
      ...
      delete: (path: string, options: RouteOptions | RouteHandler, handler?: RouteHandler) => {
        const fullPath = `${basePath}${path}`;
        if (typeof options === 'function') {
          handler = options;
          options = {};
...
<body>
  ...
  async generateOpenAPI(info: Partial<DocsMountOptions> = {}): Promise<OpenAPISpec> {
  ...
  groupWithCallback(
    basePath: string,
    optionsOrCallback: GroupOptions | ((group: GroupProxy) => void),
    callback?: (group: GroupProxy) => void
  ): ApiApp {
  ...
  health(path: string = '/health', options: HealthCheckOptions = {}): ApiApp {
  ...
  private _compileSchemaAtRegistration(
    ...
    compiledValidator: ((data: unknown) => true | ValidationError[]) | null;
    openApiRequestSchema: OpenAPISchema | null;
    openApiResponseSchema: OpenAPISchema | null;
  } {
    let compiledValidator: ((data: unknown) => true | ValidationError[]) | null = null;
    let openApiRequestSchema: OpenAPISchema | null = requestSchema as OpenAPISchema | null;
    let openApiResponseSchema: OpenAPISchema | null = responseSchema as OpenAPISchema | null;
  ...
  private _buildMiddlewareChain(route: RouteMetadata, handler: RouteHandler): HonoMiddleware[] {
    ...
    chain.push(async (c: Context): Promise<Response | void> => {
    ...
    return async (c: Context, next: Next): Promise<Response | void> => {
      ...
      for (const guard of guards) {
        try {
          ...
          return ctx.forbidden((err as Error).message);
        }
      }
  ...
  private _fvToOpenApi(schema: Record<string, unknown>): OpenAPISchema {
  ...
  private _parseRule(rule: unknown): { schema: OpenAPISchema; required: boolean } {
    ...
    if (rule && typeof rule === 'object') {
      ...
      if (ruleObj.type === 'object' && ruleObj.props) {
        const nested = this._fvToOpenApi(ruleObj.props as Record<string, unknown>);
        return {
      ...
      const schema = this._mapType((ruleObj.type as string) || 'string');

      if (ruleObj.enum) schema.enum = ruleObj.enum as unknown[];
      if (typeof ruleObj.min === 'number') schema.minimum = ruleObj.min;
      if (typeof ruleObj.max === 'number') schema.maximum = ruleObj.max;
      if (typeof ruleObj.minLength === 'number') schema.minLength = ruleObj.minLength;
      if (typeof ruleObj.maxLength === 'number') schema.maxLength = ruleObj.maxLength;
      if (ruleObj.pattern) schema.pattern = ruleObj.pattern as string;
      if (ruleObj.default !== undefined) schema.default = ruleObj.default;
  ...
  private _mapType(type: string): OpenAPISchema {
  ...
  private _generateOpenAPISpec(params: {
    title: string;
    version: string;
    description: string;
    servers?: Array<{ url: string; description?: string }>;
  ...
  private _generateAllResponses(route: RouteMetadata): Record<string, unknown> {
  ...
  private _errorSchema(includeDetails: boolean = false): OpenAPISchema {
    ...
    if (includeDetails) {
      (schema.properties!.error as OpenAPISchema).properties!.details = {
        type: 'array',
        items: {
          type: 'object',
          properties: {
  ...
  private _generateParametersFromSchema(schema: OpenAPISchema | null): Array<Record<string, unknown>> {
    ...
    for (const [key, prop] of Object.entries(schema.properties)) {
      parameters.push({
        ...
        schema: prop,
        description: (prop as OpenAPISchema).description || `${key} parameter`
      });
    }
  ...
  private _generateCodeSamples(route: RouteMetadata, baseUrl: string): Array<{ lang: string; source: string }> {
  ...
  private _generateExampleFromSchema(schema: OpenAPISchema | null): unknown {
    ...
    if (schema.type === 'object' && schema.properties) {
      ...
      for (const [key, prop] of Object.entries(schema.properties)) {
        example[key] = this._generateExampleFromSchema(prop as OpenAPISchema);
      }
      return example;
    }
