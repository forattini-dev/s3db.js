...
export function getErrorType(errors: string[] | null | undefined): ErrorType {
...
export function getErrorDetails(errorType: ErrorType, errors: string[] = []): ErrorDetails {
  const errorMap: Record<ErrorType, ErrorMapEntry> = {
    ...
    [ErrorTypes.MISSING_FIELD]: {
      title: 'Configuration Error',
      message: 'Missing required configuration field. Please contact support.',
      action: 'Contact Support',
      userAction: false
    },
    [ErrorTypes.UNKNOWN]: {
...
export function generateErrorPage(errorDetails: ErrorDetails, options: ErrorPageOptions = {}): string {
  const {
    returnUrl = '/',
    loginUrl = '/auth/login',
    supportUrl = null,
...
export function generateErrorJSON(errorDetails: ErrorDetails, statusCode: number = 401): ErrorJSONResponse {
  return {
    error: {
