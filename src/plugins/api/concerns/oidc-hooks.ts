...
const logger: Logger = createLogger({ name: 'OidcHooks', level: (process.env.S3DB_LOG_LEVEL || 'info') as LogLevel });
...
export class CookieHelpers {
  ...
  private config: CookieHelpersConfig;
  private cookiePrefix: string;
  private defaults: Required<CookieOptions>;
  ...
  private _prefixName(name: string): string {
...
export function createHookExecutor(config: HooksConfig, customLogger?: Logger): HookExecutor {
  return new HookExecutor(config, customLogger || logger);
}

export function createCookieHelpers(context: Context, config: CookieHelpersConfig = {}): CookieHelpers {
