...
export async function generateClientAssertion(options: ClientAssertionOptions): Promise<string> {
  const {
    clientId,
    tokenEndpoint,
    privateKey,
...
export async function createClientAuth(config: OidcConfig, tokenEndpoint: string): Promise<ClientAuth> {
  ...
  switch (authMethod) {
    ...
    case 'private_key_jwt':
      ...
      const assertion = await generateClientAssertion({
        ...
        tokenEndpoint,
        privateKey,
        algorithm: privateKey.alg as string || 'RS256'
      });
...
export function applyClientAuth(clientAuth: ClientAuth, requestOptions: RequestOptions): RequestOptions {
...
export function validatePrivateKey(jwk: JWK | null | undefined): PrivateKeyValidation {
  const errors: string[] = [];
...
export async function generateRSAKeyPair(options: GenerateKeyPairOptions = {}): Promise<RSAKeyPair> {
