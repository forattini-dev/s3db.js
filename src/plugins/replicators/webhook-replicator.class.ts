...
import { createHttpClient, type HttpClient } from '#src/concerns/http-client.js';
...
type ResourcesInput = string[] | Array<{ name: string; [key: string]: unknown }> | Record<string, WebhookResourceConfig | boolean>;
...
class WebhookReplicator extends BaseReplicator {
  ...
  constructor(config: WebhookReplicatorConfig, resources: ResourcesInput = [], client: unknown = null) {
    super(config);
    ...
    if (Array.isArray(resources)) {
      ...
      this.resources = resources as Record<string, WebhookResourceConfig | boolean>;
    } else {
      this.resources = {};
    }
  ...
  createPayload(resource: string, operation: string, data: unknown, id: string, beforeData: unknown = null): WebhookPayload {
  ...
  private async _getHttpClient(): Promise<HttpClient> {
    if (!this._httpClient) {
      this._httpClient = await createHttpClient({
        ...
        auth: this.auth as import('../../concerns/http-client.js').AuthConfig | undefined,
        retry: {
  ...
  private async _makeRequest(payload: unknown): Promise<WebhookRequestResult> {
  ...
  override async replicateBatch(resource: string, records: Array<{ operation: string; data: Record<string, unknown>; id: string; beforeData?: unknown }>): Promise<ReplicateResult> {
    ...
    const [ok, err, result] = await tryFn(async () => {
      ...
      const successful = results.filter(r => r.status === 'fulfilled' && (r.value as ReplicateResult).success).length;
    ...
    this.logger.warn({ resource, error: (err as Error).message }, 'Batch replication failed');
