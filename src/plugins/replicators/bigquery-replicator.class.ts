...
export interface ReplicateResult {
  ...
  skipped?: boolean;
  reason?: string;
  results?: Array<{ table: string; success: boolean; jobId?: string }>;
...
interface BigQueryClientLike {
  ...
  createQueryJob(options: { query: string; params: Record<string, unknown>; location: string }): Promise<[JobLike]>;
}

interface DatasetLike {
  getMetadata(): Promise<[unknown]>;
  table(tableName: string): TableLike;
  createTable(tableName: string, options: { schema: unknown; timePartitioning?: unknown; clustering?: unknown }): Promise<void>;
}

interface TableLike {
  exists(): Promise<[boolean]>;
  getMetadata(): Promise<[{ schema: { fields: unknown[] } }]>;
  setMetadata(metadata: { schema: unknown[] }): Promise<void>;
  delete(): Promise<void>;
...
class BigqueryReplicator extends BaseReplicator {
  ...
  private _validateMutability(mutability: string): void {
  ...
  parseResourcesConfig(resources: Record<string, ResourcesInput>): Record<string, BigQueryTableConfig[]> {
    const parsed: Record<string, BigQueryTableConfig[]> = {};
      ...
      if (typeof config === 'string') {
        ...
        parsed[resourceName] = config.map(item => {
          ...
          return {
            ...
            mutability: itemMutability as 'append-only' | 'mutable' | 'immutable',
            tableOptions: item.tableOptions || null
          };
        });
      } else if (typeof config === 'object') {
        const objConfig = config as BigQueryResourceConfig;
        ...
        parsed[resourceName] = [{
          ...
          mutability: configMutability as 'append-only' | 'mutable' | 'immutable',
  ...
  override async initialize(database: unknown): Promise<void> {
    ...
    if (!ok) {
      this.logger.warn({ error: (err as Error).message }, 'Failed to import BigQuery SDK');
      this.emit('initialization_error', { replicator: this.name, error: (err as Error).message });
      throw err;
    }
    const { BigQuery } = sdk;
    ...
    if (!connOk) {
      const errorMessage = this._parseGcpError(connErr as Error);
      const suggestion = this._getCredentialsSuggestion(connErr as Error);

      const error = this.createError(
        `BigQuery connection failed: ${errorMessage}`,
        {
          operation: 'initialize',
          statusCode: (connErr as { code?: number }).code || 401,
          retriable: true,
          suggestion
        }
      );
  ...
  async syncTableSchema(tableName: string, attributes: Record<string, unknown>, mutability: string = 'append-only', tableOptions: TableOptions | null = null): Promise<void> {
    const dataset = this.bigqueryClient!.dataset(this.datasetId);
    ...
    if (!exists) {
      ...
      const createOptions: { schema: unknown; timePartitioning?: unknown; clustering?: unknown } = { schema };
    ...
    if (this.schemaSync.strategy === 'alter' && this.schemaSync.autoCreateColumns) {
      const existingSchema = await getBigQueryTableSchema(this.bigqueryClient as any, this.datasetId, tableName);
  ...
  getTablesForResource(resourceName: string, operation: string): Array<{ table: string; transform: ((data: Record<string, unknown>) => Record<string, unknown>) | null; mutability: string; tableOptions: TableOptions | null }> {
  ...
  private _addTrackingFields(data: Record<string, unknown>, operation: string, mutability: string, id: string): Record<string, unknown> {
    const tracked = { ...data };
  ...
  private _getNextVersion(id: string): number {
  ...
  override async replicate(resourceName: string, operation: string, data: Record<string, unknown>, id: string, beforeData: unknown = null): Promise<ReplicateResult> {
    ...
    const results: Array<{ table: string; success: boolean; jobId?: string }> = [];
      ...
      for (const tableConfig of tableConfigs) {
        const [okTable, errTable] = await tryFn(async () => {
          ...
          if (operation === 'insert' || shouldConvertToInsert) {
            ...
            try {
              job = await table.insert([transformedData]) as [JobLike];
            } catch (error) {
              const { errors: bqErrors, response } = error as { errors?: unknown; response?: unknown };
              this.logger.error(
                { errors: bqErrors, response },
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
              ...
              if (okAttempt) {
                ...
                lastError = errorAttempt as Error;
            ...
            try {
              ...
              const bqErr = error as { errors?: unknown; response?: unknown };
              this.logger.error(
                { query, errors: bqErr.errors, response: bqErr.response },
    ...
    this.logger.warn(
      { resourceName, error: (err as Error).message },
      'Replication failed'
    );
    ...
    const { results, errors } = await this.processBatch(
      ...
      },
      {
        ...
        mapError: (error, record) => {
          ...
          this.logger.warn(
            { recordId: rec.id, error: (error as Error).message },
            'Batch replication failed for record'
          );
          return { id: rec.id, error: (error as Error).message };
        }
      }
    );
  ...
  private _parseGcpError(error: Error): string {
  ...
  private _getCredentialsSuggestion(error: Error): string {
    const message = error.message || String(error);
  ...
  override async testConnection(): Promise<boolean> {
    const [ok, err] = await tryFn(async () => {
      if (!this.bigqueryClient) await this.initialize(this.database);
      const dataset = this.bigqueryClient!.dataset(this.datasetId);
      await dataset.getMetadata();
      return true;
    });
