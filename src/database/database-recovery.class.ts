...
export class DatabaseRecovery {
  ...
  async attemptJsonRecovery(content: string, healingLog: string[]): Promise<SavedMetadata | null> {
  ...
  async validateAndHealMetadata(metadata: SavedMetadata, healingLog: string[]): Promise<SavedMetadata> {
    ...
    const validResources: StringRecord<ResourceMetadata> = {};
  ...
  private _healResourceStructure(name: string, resource: ResourceMetadata, healingLog: string[]): ResourceMetadata | null {
  ...
  private _healHooksStructure(hooks: StringRecord<HookSummary>, resourceName: string, healingLog: string[]): StringRecord<HookSummary> {
    if (!hooks || typeof hooks !== 'object') {
      healingLog.push(`Resource ${resourceName}: invalid hooks structure - using empty hooks`);
      return {};
    }

    const healed: StringRecord<HookSummary> = {};
    let changed = false;

    for (const [event, hookData] of Object.entries(hooks)) {
      if (hookData && typeof hookData === 'object' && Array.isArray(hookData.handlers)) {
        const validHandlers = hookData.handlers.filter((handler: any) =>
          handler !== null &&
          handler !== undefined &&
          handler !== ''
        );
  ...
  async createCorruptedBackup(content: string | null = null): Promise<void> {
    const [ok] = await tryFn(async () => {
      ...
      if (!content) {
        ...
        content = readOk ? (readData as string) : 'Unable to read corrupted file content';
      }
  ...
  async uploadHealedMetadata(metadata: SavedMetadata, healingLog: string[]): Promise<void> {
    ...
    if (!ok) {
      this.database.logger.error({ error: (err as Error)?.message }, 'failed to upload healed metadata');
      throw err;
    }
  }
}
