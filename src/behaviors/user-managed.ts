...
export async function handleInsert({ resource, data, mappedData, originalData }: BehaviorHandleInsertParams): Promise<BehaviorResult> {
...
export async function handleUpdate({ resource, id, data, mappedData, originalData }: BehaviorHandleUpdateParams): Promise<BehaviorResult> {
  const totalSize = calculateTotalSize(mappedData);
...
export async function handleUpsert({ resource, id, data, mappedData }: BehaviorHandleUpsertParams): Promise<BehaviorResult> {
  const totalSize = calculateTotalSize(mappedData);
...
export async function handleGet({ metadata, body }: BehaviorHandleGetParams): Promise<BehaviorGetResult> {
  if (body && body.trim() !== '') {
    const [ok, , result] = await tryFn(() => {
      const bodyData = JSON.parse(body) as StringRecord<string>;
      return {
        metadata: {
          ...bodyData,
          ...metadata
    ...
    if (ok) {
      return result as BehaviorGetResult;
    }
  }
