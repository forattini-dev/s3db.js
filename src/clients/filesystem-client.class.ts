...
const globalStorageRegistry = new Map<string, FileSystemStorage>();
...
export class FileSystemClient extends EventEmitter {
  ...
  constructor(config: FileSystemClientConfig = {}) {
    ...
    if (config.logger) {
      ...
      this.logger = createLogger({ name: 'FileSystemClient', level: this.logLevel as LogLevel });
    }
  ...
  private async _handleListObjects(input: CommandInput): Promise<ListObjectsResponse> {
    ...
    if (input.StartAfter) {
      params.startAfter = this._applyKeyPrefix(input.StartAfter as string);
    }
