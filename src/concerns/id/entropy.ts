 ...
 */
export function randomIndexUnbiased(alphabetSize: number): number {
 ...
 */
export function random48(): bigint {
 ...
 */
export function random62(): bigint {
  const bytes = getRandomBytes(8);
  return (
    (BigInt(bytes[0]! & 0x3f) << 56n) |
    (BigInt(bytes[1]!) << 48n) |
    (BigInt(bytes[2]!) << 40n) |
    (BigInt(bytes[3]!) << 32n) |
    (BigInt(bytes[4]!) << 24n) |
 ...
 */
export function random80(): bigint {
 ...
 */
export function calculateCollisionProbability(entropyBits: number, idCount: number): number {
