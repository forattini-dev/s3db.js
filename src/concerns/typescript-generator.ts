import { createLogger, S3DBLogger } from './logger.js';
import { writeFile, mkdir } from 'fs/promises';
import { dirname } from 'path';

const logger: S3DBLogger = createLogger({ name: 'TypeScriptGenerator', level: 'info' });
...
function mapFieldTypeToTypeScript(fieldType: string): string {
...
function isFieldRequired(fieldDef: FieldDefinition): boolean {
...
function generateResourceInterface(
  resourceName: string,
  attributes: Record<string, FieldDefinition>,
      ...
      if (fieldDef.type === 'object' && fieldDef.props) {
        ...
        for (const [propName, propDef] of Object.entries(fieldDef.props)) {
          const propType = typeof propDef === 'string'
            ...
            : mapFieldTypeToTypeScript((propDef as ObjectFieldDefinition).type || 'any');
...
export async function generateTypes(
  ...
  const resourceInterfaces: ResourceInterface[] = [];
  ...
  for (const [name, resource] of Object.entries(database.resources)) {
    ...
    const userAttributes = Object.fromEntries(
      Object.entries(allAttributes).filter(([attrName]) => !pluginAttrNames.includes(attrName))
    );
