...
export function encodeGeoLat(lat: number | null | undefined, precision: number = 6): string | number | null | undefined {
...
export function decodeGeoLat(encoded: string | number, precision: number = 6): number | string {
...
export function encodeGeoLon(lon: number | null | undefined, precision: number = 6): string | number | null | undefined {
...
export function decodeGeoLon(encoded: string | number, precision: number = 6): number | string {
  if (typeof encoded !== 'string') return encoded;
  if (!encoded.startsWith('~')) return encoded;
...
export function encodeGeoPoint(lat: number, lon: number, precision: number = 6): string {
...
export function decodeGeoPoint(encoded: string, precision: number = 6): GeoPoint {
  ...
  const latitude = decodeGeoLat('~' + parts[0], precision) as number;
  const longitude = decodeGeoLon('~' + parts[1], precision) as number;
...
export function getPrecisionForAccuracy(accuracyMeters: number): number {
...
export function getAccuracyForPrecision(precision: number): number {
