...
export function expandIPv6(ip: string): string {
  ...
  if (expanded.includes('::')) {
    ...
    const middleParts = Array(missingGroups).fill('0') as string[];
...
export function compressIPv6(ip: string): string {
  ...
  interface ZeroSequence {
    start: number;
    length: number;
  }

  const zeroSequences: ZeroSequence[] = [];
  let currentSequence: ZeroSequence = { start: -1, length: 0 };
 ...
 */
export function decodeIPv6(encoded: string, compress = true): string {
  if (typeof encoded !== 'string') {
 ...
 */
export function calculateIPSavings(ip: string): IPSavingsResult {
