...
export class FilesystemStorageDriver {
  ...
  async set(key: string, data: Record<string, unknown>, options: SetOptions = {}): Promise<SetResult> {
    ...
    if (!ok) {
      throw new PluginStorageError(`Failed to save data: ${(err as Error).message}`, {
        operation: 'set',
        key,
        path: filePath,
  ...
  async get(key: string): Promise<Record<string, unknown> | null> {
    ...
    if (!ok) {
      throw new PluginStorageError(`Failed to read data: ${(err as Error).message}`, {
        operation: 'get',
        key,
        path: filePath,
    ...
    try {
      ...
      throw new PluginStorageError(`Failed to parse JSON: ${(parseErr as Error).message}`, {
        operation: 'get',
        key,
        path: filePath,
  ...
  async delete(key: string): Promise<boolean> {
    ...
    if (!ok) {
      throw new PluginStorageError(`Failed to delete data: ${(err as Error).message}`, {
        operation: 'delete',
        key,
        path: filePath,
        suggestion: 'Check filesystem permissions'
  ...
  async list(options: ListOptions = {}): Promise<string[]> {
    ...
    const walk = async (dir: string, _currentPrefix: string = ''): Promise<void> => {
