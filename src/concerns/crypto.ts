...
import tryFn, { tryFnSync, TryResult } from './try-fn.js';
...
async function dynamicCrypto(): Promise<WebCrypto> {
  let lib: WebCrypto | undefined;

  if (typeof process !== 'undefined') {
    lib = crypto.webcrypto as unknown as WebCrypto;
  } else if (typeof window !== 'undefined') {
    lib = window.crypto as WebCrypto;
  }
...
export async function sha256(message: string): Promise<string> {
  const [okCrypto, errCrypto, cryptoLib] = await tryFn<WebCrypto>(dynamicCrypto);
  if (!okCrypto) throw new CryptoError('Crypto API not available', { original: errCrypto });

  const encoder = new TextEncoder();
  const data = encoder.encode(message);
  const [ok, err, hashBuffer] = await tryFn<ArrayBuffer>(() => cryptoLib.subtle.digest('SHA-256', data));
...
export async function encrypt(content: string, passphrase: string): Promise<string> {
  const [okCrypto, errCrypto, cryptoLib] = await tryFn<WebCrypto>(dynamicCrypto);
  if (!okCrypto) throw new CryptoError('Crypto API not available', { original: errCrypto });

  const salt = cryptoLib.getRandomValues(new Uint8Array(16));
  const [okKey, errKey, key] = await tryFn<CryptoKey>(() => getKeyMaterial(passphrase, salt));
  ...
  const [okEnc, errEnc, encryptedContent] = await tryFn<ArrayBuffer>(() =>
    cryptoLib.subtle.encrypt({ name: 'AES-GCM', iv: iv }, key as CryptoKey, encodedContent)
  );
...
export async function decrypt(encryptedBase64: string, passphrase: string): Promise<string> {
  ...
  const [okDec, errDec, decryptedContent] = await tryFn<ArrayBuffer>(() =>
    cryptoLib.subtle.decrypt({ name: 'AES-GCM', iv: iv as BufferSource }, key as CryptoKey, encryptedContent as BufferSource)
  );
...
async function getKeyMaterial(passphrase: string, salt: Uint8Array): Promise<CryptoKey> {
  const [okCrypto, errCrypto, cryptoLib] = await tryFn<WebCrypto>(dynamicCrypto);
  if (!okCrypto) throw new CryptoError('Crypto API not available', { original: errCrypto });
  ...
  const [okImport, errImport, baseKey] = await tryFn<CryptoKey>(() => cryptoLib.subtle.importKey(
    'raw',
    keyMaterial,
    { name: 'PBKDF2' },
    false,
  ...
  const [okDerive, errDerive, derivedKey] = await tryFn<CryptoKey>(() => cryptoLib.subtle.deriveKey(
    {
      name: 'PBKDF2',
