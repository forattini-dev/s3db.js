...
import { getCronManager, CronTask } from './cron-manager.js';
...
interface ResourceLike {
  ...
  list(options?: { limit?: number }): Promise<BanRecord[]>;
...
interface DatabaseLike {
  ...
  pluginRegistry?: {
    ttl?: {
      options: {
        resources?: Record<string, { enabled: boolean; field: string }>;
      };
    };
    TTLPlugin?: {
      options: {
...
export class FailbanManager {
  ...
  private async _createBansResource(): Promise<ResourceLike> {
  ...
  private async _createViolationsResource(): Promise<ResourceLike> {
  ...
  private async _initializeGeoIP(): Promise<void> {
    ...
    try {
      const maxmind = await requirePluginDependency(
        '@maxmind/geoip2-node',
        'ApiPlugin (Failban)'
      );
      const Reader = maxmind as unknown as { open(path: string): Promise<GeoReader> };
